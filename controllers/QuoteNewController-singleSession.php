...
{	
	...
	public function init()
	{
		...
		/*webservice links*/
		$this->_view->crossDomain=$this->crossDomain='http://admin-ep.edit-place.co.uk';
	...
	function saveQuoteCommentsAction()
	{
		if($this->_request-> isPost())            
        {
        	...
        	$quote_log['action']="quote_forum";
			$quote_log['comments']=htmlentities($commentsParams['quotes_forum_comments']);
        	...
        	if($commentsParams['comment_action']=='update')
        	{
				...
				$quote_update_log['comments']=htmlentities($commentsParams['quotes_forum_comments']);				
        	...
        	//echo "<pre>"; print_r($commentDetails); exit;
        	$commentsData='';
						...
						$commentsData.='	
							<div class="action-comment markeddown js-comment">
								...
								<div class="comment-box" id="edit_user_comment_'.$comment['id'].'" style="display:none">
									<textarea tabindex="1" class="comment-box-input js-text" name="quotes_forum_comments_'.$comment['id'].'" id="quotes_forum_comments_'.$comment['id'].'" rel="'.$comment['quote_id'].'">'.html_entity_decode($comment['comments']).'</textarea>
				...
				else
				{
					...
					if($quote_missions[$i]['selected_mission'])
					{
						...
						if((is_array($missionDetails) && count($missionDetails) < 1)|| (!$missionDetails))
						{
							...
							/*build service link to get HOQ prices from UK*/
							$service_link=$this->web_service_hoq_prices_link;
							//get details from UK with service link
							...
							if($quoteMissionId)
							{
								...
								if($quoteMission_data['sales_suggested_missions'] && $quoteMissionId)
								{
									...
									if(count($prod_detailsexsist)>0)
								    {
										...
										/*getting Prod mission details from UK*/
										if(count($prod_details)==0 && !is_array($prod_details))
										{
											/*build service link to get prod mission prices from UK*/
											...
											$quoteMission_data['suggested_mission_from']='uk';
										}									
		...
		if(is_array($this->quote_creation->create_mission['product']) && count($this->quote_creation->create_mission['product'])>0 || count($this->quote_creation->tech_mission['product'])>0)
		{
			...
			foreach($this->quote_creation->create_mission['product'] as $mission)
			{
				...
				$quote_missions[$i]['internal_cost']=$this->quote_creation->create_mission['internal_cost'][$i];
					if(!$quote_missions[$i]['internal_cost'])$quote_missions[$i]['internal_cost']=0;	
				$quote_missions[$i]['turnover']=$quote_missions[$i]['unit_price']*$quote_missions[$i]['volume'];
				$quote_missions[$i]['team_packs']=$this->quote_creation->create_mission['team_packs'][$i];
			...
			{				
				...
				if($missonDetails)
				{
					...
					foreach($missonDetails as $quoteMmission)
					{
						...
						$this->quote_creation->create_mission['team_packs'][$i]=$quoteMmission['team_packs'];
	...
	function historyMissionDetailsAction()
	{
		...
		else if($mission_id && $from_site=='uk')
		{
			/*build service link to get HOQ prices from UK*/
	...
	public function quoteMissoinUpdate($quoteMissionId,$offsetid)
	{
		...
		$id=$offsetid;
			...
			//echo $quoteMissionIdentifier.'Test'; exit;
	...
	function getHoqPricesAction()
	{
		...
		if($mission_id && isset($mindex))
		{
			...
			if($this->quote_creation->create_mission['product'][$i])
			{
				/*Added w.r.t Autre mission*/
				if($this->quote_creation->create_mission['product'][$i]=='autre')
				{
	...
	{
		$missionParams=$this->_request->getParams();		
		$showtheorical=$this->quote_creation->create_mission['showtheorical']? $this->quote_creation->create_mission['showtheorical'] : 'no';
		$mission_id=$missionParams['mission_id'];
		$mindex=$missionParams['mindex'];
		if($mission_id && isset($mindex) && $showtheorical)
		{
			...
			if($showtheorical=='yes')
				$quoteMission_data['sales_suggested_missions']=$this->quote_creation->create_mission['selected_mission'][$mindex]=$missionParams['overview_missions_'.$mission_id];
			else
				$quoteMission_data['sales_suggested_missions']=$this->quote_creation->create_mission['selected_mission'][$mindex]=$missionParams['roverview_missions_'.$mission_id];	
			
			$quoteMission_data['margin_percentage']=$this->quote_creation->create_mission['margin_percentage'][$mindex]=$missionParams['margin_percentage_'.$mission_id];
			
			$quoteMission_data['unit_price']=$this->quote_creation->create_mission['unit_price'][$mindex]=$missionParams['unit_price_'.$mission_id];
			
			$quoteMission_data['internal_cost']=$this->quote_creation->create_mission['internal_cost'][$mindex]=$missionParams['internal_cost_'.$mission_id];
	...
	//connect to UK server with a link to get details or connect to other sites
