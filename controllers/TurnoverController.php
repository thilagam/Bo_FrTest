...
class TurnoverController extends Ep_Controller_Action
{
	...
	function realMonthAction()
	{
		...
		foreach($realturnovers as $realturnover)
		{
            ...
            if(trim($realturnover['ind_currency']) === 'euro')
                $monthturnovers[$realturnover['user_id']]['euro'][$realturnover['yearmonth']] = $realturnover['total'];
            else
                $monthturnovers[$realturnover['user_id']]['pounds'][$realturnover['yearmonth']] = $realturnover['total'];
		...
		$realseoturnovers = $turnover_obj->getRealSeoMission($request);//echo "<pre>";print_r($realseoturnovers);exit;
		foreach($realseoturnovers as $realturnover)
		{
			...
			$monthturnovers[$realturnover['user_id']]['other_info'] = $realturnover;
            if(trim($realturnover['ind_currency']) === 'euro')
                $monthturnovers[$realturnover['user_id']]['euro'][$realturnover['yearmonth']] += $realturnover['missionturnover'];
            else
                $monthturnovers[$realturnover['user_id']]['pounds'][$realturnover['yearmonth']] += $realturnover['missionturnover'];
							...
							if($request['year']==$missionTastdetails['validatedyear'])
							{
								...
								if($realturnover['package']=='team')
									...
									$monthturnovers[$realturnover['user_id']][$missionTastdetails['validatedyearmonth']] += $techtask['turnover'];
									 ...
									 if(trim($realturnover['ind_currency']) === 'euro')
						                $monthturnovers[$realturnover['user_id']]['euro'][$missionTastdetails['validatedyearmonth']] += $techtask['turnover'];
						            else
						                $monthturnovers[$realturnover['user_id']]['pounds'][$missionTastdetails['validatedyearmonth']] += $techtask['turnover'];
		...
		foreach($real_client as $client_list)
		{
			...
			if($client_list['ind_currency']=='euro')
			{
			$realtotalturnovereuro[$client_list['user_id']] += $client_list['total'];
			$realtotalturnovereurovar+= $client_list['total'];
			}
			else
			{
			$realtotalturnoverpound[$client_list['user_id']]+= $client_list['total'];
			$realtotalturnoverpoundvar+= $client_list['total'];
			}
			if($realtotalturnoverpound[$client_list['user_id']]==0 && $realtotalturnovereuro[$client_list['user_id']]==0)
			{
		...
		foreach($real_seo_clients as $client_list)
		{
			...
			if($client_list['ind_currency']=='euro')
			{
			$realtotalturnovereuro[$client_list['user_id']]+= $client_list['missionturnover'];
			$realtotalturnovereurovar+= $client_list['missionturnover'];
			}
			else
			{
			$realtotalturnoverpound[$client_list['user_id']]+= $client_list['missionturnover'];
			$realtotalturnoverpoundvar+= $client_list['missionturnover'];
			}
						...
						foreach($techtaskMission as $missionTastdetails)
						{
							if($request['year']==$missionTastdetails['validatedyear'])
							{
								...
								if($realturnover['package']=='team')
								 ...
								 if($client_list['ind_currency']=='euro')
									{
									$realtotalturnovereuro[$client_list['user_id']] += $techtask['turnover'];
									$realtotalturnovereurovar+=$techtask['turnover'];
									}
									else
									{
									$realtotalturnoverpound[$client_list['user_id']] += $techtask['turnover'];
									$realtotalturnoverpoundvar+= $techtask['turnover'];
									}
							}
		...
		$this->_view->totalturnovereuro = $realtotalturnovereurovar;
		$this->_view->totalturnoverpound = $realtotalturnoverpoundvar;
		natcasesort($salesrealDetails);
		$this->_view->salesusers = $salesrealDetails;
		//$this->_view->clients = $contract_obj->getUsers('client');
        if(isset($_REQUEST['err']) && $_REQUEST['err'] === "yes"){
           $this->_view->err = "yes";}
        else{
            $this->_view->err = "no";}
		...
		if($request['client']&&$request['year'])
		{
			...
			foreach($contracts as $contract)
			{
				...
				/* Real turnover and missions */
				 $filters_product = $turnover_obj->getProdSeoMissions(array('contract_id'=>$contract['quotecontractid'],'cm_status'=>$request['status']));
				...
				$techParameters['cm_status']=$request['status'];
				...
				/*End Filters*/
				$cmissiondetails = $turnover_obj->getProdSeoMissions(array('contract_id'=>$contract['quotecontractid'],'product'=>$request['product'],'product_type'=>$request['p_type'],'cm_status'=>$request['status']));
				//echo "<pre>"; print_r($cmissiondetails); exit;
				foreach($cmissiondetails as $contract_mission)
				{
					...
					{
						$years_mission = $turnover_obj->getRealTurnovers(array('contract_mission_id'=>$contract_mission['contractmissionid'],'year'=>$year,'cm_status'=>$request['status']));
			...
			//	exit;
				...
				$searchParameters['cm_status']=$request['status'];
				//echo '<pre>'; print_r($searchParameters); exit;
				...
				foreach($techMissionDetails as $tech_mission)
				{
					...
					{
						$techtaskMission = $task_obj->getTechTaskMissions(array('cmid'=>$tech_mission['contractmissionid'],'m_status'=>'validated','cm_status'=>$request['status']));
				...
				$searchcontract['cm_status']=$request['status'];
		...
		{ //split turnover for contract 
			...
			if(!in_array($splitbyclient['mission_id'],$clienttrunarray[$clientloop['quotecontractid']]))
				...
				if($request['pm']=='true')
				{
					...
					$contractMissoinType= $turnover_obj->getMissionDetails($splitbyclient['mission_id'],array('product'=>$request['product'],'cid'=>$clientloop['quotecontractid'],'assigned_to'=>$assigned_toval,'cm_status'=>$request['status']));
						if(!$contractMissoinType)
						{
							...
							if($techMissoinType)
							{
								...
								$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']]['cm_status']=$techMissoinType[0]['cm_status'];
							}
							...
							$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['mission_id']]['cm_status'] = $contractMissoinType[0]['cm_status'];
			...
			{	
					if($year_montharray[0]==$default_year )
					{
						...
						$clienttrunarray[$clientloop['quotecontractid']][$splitbyclient['month_year']]=$splitbyclient['turnover'];
							...
							$contractMissoinType= $turnover_obj->getMissionDetails($splitbyclient['mission_id'],array('product'=>$request['product'],'cid'=>$clientloop['quotecontractid'],'assigned_to'=>$request['assigned_to'],'cm_status'=>$request['status']));
							if(!$contractMissoinType)
							{
							$searchParameters['contract_id']= $clientloop['quotecontractid'];
							$searchParameters['include_final']='yes';
	...
	function contractMonthlyDetailsAction($month_year,$client_id)	
	{
		...
		$clientcontractlist = $contract_obj->getClientContracts(array('client_id'=>$parmas['client_id'],'status'=>$parmas['status']));
		...
		foreach($clientcontractlist as $clientloop)
		{
			...
			if($splitmonthturnoverclient=="")
				{
									 ...
									 $missionDetailsloop=$contract_obj->getProdSeoMissions(array('contract_id'=>$clientloop['quotecontractid'],'product'=>$parmas['product'],'product_type'=>$parmas['p_type'],'cm_status'=>$parmas['status']));
										...
										$searchParameters['contract_id']= $clientloop['quotecontractid'];
										$searchParameters['include_final']='yes';
										$searchParameters['cm_status']=$parmas['status'];
				...
				{	
					...
					{ //split turnover for contract 
						...
						if($year_montharray[0]==$default_year)
						{
							...
							//get Mission details
							$contractMissoinType= $contract_obj->getMissionDetails($splitbyclient['mission_id'],array('product'=>$request['product'],'cid'=>$clientloop['quotecontractid'],'cm_status'=>$parmas['status']));		
							$clientloop['expected_end_date']=date('Y-m-d', strtotime($clientloop['expected_launch_date']. ' + '.$contractMissoinType[0]['mission_length'].' days'));
			...
			$turnover_obj = new Ep_Quote_Turnover();
			$quotecontract_obj = new Ep_Quote_Quotecontract();
			$cmissiondetails = $turnover_obj->getProdSeoMissions(array('contract_id'=>$clientloop['quotecontractid'],'cm_status'=>$parmas['status']));
			foreach($cmissiondetails as $contract_mission)
			{
				if($contract_mission['contractmissionid'] && $contract_mission['type']=="prod")
				{
			...
			foreach($techMissionDetails as $tech_mission)
			{
				...
				if($default_year==$tech_mission['validatedyear'] && $default_month==$tech_mission['validatedmonth'] && count($techtaskMission)==0 && $tech_mission['contractmissionid']!="")
				{
					...
					if(count($techtaskMission)>0 && $tech_mission['contractmissionid']!="")
					{
						$techtask['turnover']=0;
						foreach($techtaskMission as $missionTastdetails)
						{
	...
	function splitMonthTurnoverAction()
	{
					...
					foreach($clientcontractlist as $clientloop)
						{
							...
							$missionDetailsloop=$turnover_obj->getProdSeoMissions(array('contract_id'=>$clientloop['quotecontractid'],'cm_status'=>$contractParam['status']));
								 foreach($missionDetailsloop as $missoin)
									{
									 if($missoin['product']=='translation')
									{	
							...
							$techParameters['include_final']='yes';
							$techParameters['cm_status']=$contractParam['status'];
							$techMissionDetails=$turnover_obj->getTechMissionDetails($techParameters);
							...
							foreach($techMissionDetails as $techfilmisson)
							 {
							   ...
							   //client Details
								...
								if($contractParam['product'])
								{
									...
									$searchcontract['cm_status']=$contractParam['status'];
								$splitmonthturnoverclient=$turnover_obj->getSplitTurnoversclients($searchcontract);
							...
							if($splitmonthturnoverclient=="")
								 ...
								 $missionDetailsloop=$turnover_obj->getProdSeoMissions(array('contract_id'=>$clientloop['quotecontractid'],'product'=>$contractParam['product'],'product_type'=>$contractParam['p_type'],'cm_status'=>$contractParam['status']));
								 $c=0;
										 foreach($missionDetailsloop as $missoin)
										 {
											 if(!in_array($missoin['qmid'],$clienttrunarray[$clientloop['quotecontractid']]))
											...
											$clienttrunarray[$clientloop['quotecontractid']][$missoin['qmid']]['cm_status'] = $missoin['cm_status'];
											$clienttrunarray[$clientloop['quotecontractid']][$missoin['qmid']]['assigned_to'] = $missoin['assigned_to'];
											$clienttrunarray[$clientloop['quotecontractid']][$missoin['qmid']]['assigned_name'] = $missoin['first_name']." ".$missoin['last_name'];
											$c++; 
										 }
										...
										$searchParameters['contract_id']= $clientloop['quotecontractid'];
										$searchParameters['include_final']='yes';
										$searchParameters['cm_status']=$contractParam['status'];
										...
										foreach($techMissionDetails as $techmisson)
										 {
											...
											$clienttrunarray[$clientloop['quotecontractid']][$techmisson['identifier']]['cm_status'] = $missoin['cm_status'];
											$clienttrunarray[$clientloop['quotecontractid']][$techmisson['identifier']]['mission_type'] = $techmisson['title'];
											$clienttrunarray[$clientloop['quotecontractid']][$techmisson['identifier']]['assigned_to'] = $techmisson['assigned_to'];
											$clienttrunarray[$clientloop['quotecontractid']][$techmisson['identifier']]['assigned_name'] = $techmisson['first_name']." ".$techmisson['last_name'];
		...
		$c=0;
		if($contractParam['year'])
		$contractYear['year']=$contractParam['year'];
		else
		$contractYear['year']=date("Y");

		$contractlist = $turnover_obj->getClientContracts($contractYear);
		...
		foreach($contractlist as $search_list)
		{
			...
			foreach($splitmonthturnovertotal as $splitturn)
			{
				...
				$year_total=explode('-',$splitturn['month_year']);
					...
					{
						if(trim($splitturn['ind_currency']) ==='euro')
						{
						...
						$totalclientturnover[$search_list['client_id']]+=$splitturn['turnover'];
					}

					
			}

			if($totalclientturnover[$search_list['client_id']]==0)
			{
			unset($client_list[$search_list['client_id']]);
		...
		{ //start
			...
			$splitmonthturnover=$turnover_obj->getSplitTurnovers($quotescon['quotecontractid']);
					...
					{ //split turnover loop start
						...
						if($year_montharray[0]==$this->_view->default_year)
						{
							/* *** edited on 12.02.2016 *** */
                            // calculation according to currency based on each mission//
							if(array_key_exists($splitturnover['month_year'],$spliarray[$quotescon['client_id']]))
							{
							    ...
							    if(trim($splitturnover['ind_currency']) === 'euro')
                                    $spliarray[$quotescon['client_id']]['euro'][$splitturnover['month_year']]=$spliarray[$quotescon['client_id']]['euro'][$splitturnover['month_year']]+$splitturnover['turnover'];
							    else
                                    $spliarray[$quotescon['client_id']]['pounds'][$splitturnover['month_year']]=$spliarray[$quotescon['client_id']]['pounds'][$splitturnover['month_year']]+$splitturnover['turnover'];
							...
							else
							{
                                ...
                                if(trim($splitturnover['ind_currency']) === 'euro')
                                    $spliarray[$quotescon['client_id']]['euro'][$splitturnover['month_year']]=$splitturnover['turnover'];
                                else
							        $spliarray[$quotescon['client_id']]['pounds'][$splitturnover['month_year']]=$splitturnover['turnover'];

							}
                            $spliarray[$quotescon['client_id']]['ind_currency'][$splitturnover['month_year']]=$splitturnover['ind_currency'];
                            if(trim($splitturnover['ind_currency']) === 'euro')
                                    $spliarray[$quotescon['client_id']]['euro']['turnover']=$spliarray[$quotescon['client_id']]['euro']['turnover']+$splitturnover['turnover'];
                                else
                                    $spliarray[$quotescon['client_id']]['pounds']['turnover']=$spliarray[$quotescon['client_id']]['pounds']['turnover']+$splitturnover['turnover'];
                            // for loop to calculate the total monthly wise and save in an array//

                            /* end of edited on 12.02.2016 */
					...
					if($spliarray[$quotescon['client_id']]['pounds']['turnover']==0 && $spliarray[$quotescon['client_id']]['euro']['turnover']==0)
					{
	...
	function splitMonthReportAction()
	{
		...
		if($parmas['client_id']!="")
		{
				...
				$searchclient['status']=$parmas['status'];
							...
							foreach($clientdetails as $key=>$missiondetails)
								{
									...
									{			
										$tablehtml .="<tr>";
										$tablehtml .="<td></td>";
										$tablehtml .="<td color='#267cff'>".$clientdetails[$key]['mission_type']."</td>";
										$tablehtml .="<td></td>";
				...
				foreach($contractturnover[$client_id] as $contract)
				{
					...
					foreach ($contract as $missionkey=>$mission)
					{
						...
						else
						{
							...
							$tablehtml .="<td $bgcolor>".$mission['title']."</td>";
							$tablehtml .="<td $bgcolor></td>";
							$tablehtml .="<td $bgcolor></td>";
							$tablehtml .="<td $bgcolor></td>";
							$mistotal=0;
...
//echo $tablehtml;
    ...
    /* *** added on 03.02.2016 *** */
    // function to download sales report xlsx files of particular month  //
    function downloadSalesReportXlsxAction()
    {
        $cron_obj = new Ep_Quote_Cron();
        $tech_obj = new Ep_Quote_TechMissions();
        $turnover_obj = new Ep_Quote_Turnover();
        $first_date = $_REQUEST['year'].'-'.$_REQUEST['month'].'-01';//date('Y-m-d',strtotime('first day of last month'));
        $last_date = date('Y-m-t',strtotime($first_date));
        $result = $turnover_obj->getExtract($first_date,$last_date);
        ...
        if($result)
        {
            ...
            $htmltable ="<table>
					...
					<td><b>PO name</b></td>
					".( ($old_td === true)? "<td><b>PO URL</b></td>" : "")."
					<td><b>Mission</b></td>
					<td><b>Language</b></td>
					".( ($old_td === true)? "<td><b>Tempo</b></td>" : "")."
            ...
            foreach($result as $row)
            {
                ...
                if($row['oneshot']=='yes')
                    $tempo='Oneshot';
                else if($row['oneshot']=='no')
                {
                    if($row['tempo']=='fix')
                        $tempo='Fixe';
                    else if($row['tempo']=='max')
                        $tempo='Max';
                }
                ...
                $htmltable .= "<tr>
								...
								".( ($old_td === true) ? "<td >http://".$_SERVER['HTTP_HOST']."/contractmission/contract-edit?submenuId=ML13-SL3&contract_id=".$row['quotecontractid']."&action=view</td>" : "")."
								<td >".$mtitle."</td>
								<td >".$language."</td>
								".( ($old_td === true)? "<td >".$tempo."</td>" : "")."
								<td >".$row['tot_pub_art']."</td>
								<td >".$row['unit_price']."</td>
								<td ></td>
								<td >".$row['writer_cost']."</td>
            ...
            $filename = "wp-FR-extract-".date('d-M-Y',strtotime($first_date))."-".date('d-M-Y',strtotime($last_date)).".xlsx";
            $cfilename = $path.$filename;
            $this->convertHtmltableToXlsx($htmltable,$cfilename,true);
            $this->_redirect("/BO/download-files.php?function=downloadSalesReportXlsx&filename=$filename");
            //$this->sendMail("comptabilite@edit-place.com", "Sales Report for the month of $previous_month $year at FR PTF", "PFA", $path, $filename,"rakeshm@edit-place.com");
            //$this->sendMail("nass0069@gmail.com", "Sales Report for the month of $previous_month $year at FR TEST", "PFA", $path, $filename,"naseer.pasha.n@gmail.com");
            //unlink($cfilename);
        }
        else{
            $this->_redirect("/turnover/real-month?submenuId=ML13-SL8&err=yes");
        }
    }
