...
class QuoteTestController extends Ep_Controller_Action
{
	...
	function getExtract2Action()
	{
		$ongoing_obj = new Ep_Quote_Ongoing();
		$tech_obj = new Ep_Quote_TechMissions();
		$result = $ongoing_obj->getExtract2();
		if($result)
		{
			$i = 0;
			$htmltable ="<table>
			...
			foreach($result as $row)
			{
				...
				$seoturnover = $ongoing_obj->getSeoTurnover($row['quote_id']);
	...
	function getJLAction()
	{
		$ongoing_obj = new Ep_Quote_Ongoing();
		$result = $ongoing_obj->getExtract3();
		if($result)
		{
			$i = 0;
			$htmltable ="<table>
					<tr>
						<th colspan='4'></th>
						<th colspan='8' bgcolor='#6AA121' align='center'>Writers</th>
						<th colspan='8' bgcolor='#FFFF00' align='center'>Proofreaders</th>
					</tr>
					<tr>
						...
						<th>delievery</th>
						<th>Launch date</th>
						<th>High cost new delivery</th>
						<th>Low cost new delivery</th>
						<th>price validate by the project manager</th>
						<th>Writers name</th>
						<th>Selection writer date</th>
						<th>Date of deposite by the writer</th>
						<th>Relaunch (Yes/No)</th>
						<th>Selection automatique (Yes/No)</th>
						<th>High cost new delivery</th>
						<th>Low cost new delivery</th>
						<th>Price validate by the project manager</th>
						<th>Proofreaders name</th>
						<th>Selection proofreader date</th>
						<th>Date of deposite by the proofreader</th>
						<th>Relaunch (Yes/No)</th>
						<th>Selection automatique (Yes/No)</th>
					</tr>	
			...
			foreach($result as $row)
			{
				...
				$htmltable .= "<tr>
								...
								<td >".$row['price_max']."</td>
								<td >".$row['price_min']."</td>
								<td >".$row['price_user']."</td>
								<td >".$row['writer_name']."</td>
								<td >".$row['writer_sdate']."</td>
								<td >".$row['article_sent_at']."</td>
								<td >".$row['relaunch']."</td>
								<td >".$row['stype']."</td>";
								if($row['correction']=="external")
								{
															
								$htmltable .=  "<td >".$row['correction_pricemax']."</td>
								<td >".$row['correction_pricemin']."</td>
								<td >".$row['price_corrector']."</td>
								<td >".$row['corrector_name']."</td>
								<td >".$row['corrector_sdate']."</td>
								<td >".$row['corrector_article_sent_at']."</td>
								<td >".$row['crelaunch']."</td>
								<td >".$row['sctype']."</td>
								";	
								}
								...
								{
									$htmltable .=  "<td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td>";
								}
			...
			$cfilename = "extract-Julie-Lorna-".time().".xlsx";
			$this->convertHtmltableToXlsx($htmltable,$cfilename,true);
			$this->_redirect("/BO/download-quote-xls.php?session_id=".$cfilename);
		}
	}
