...
class ContractController extends Ep_Controller_Action
{
    public function init()
    {
		...
		$this->spec_path="/home/sites/site6/web/BO/contract_spec/";
        $this->article_path="/home/sites/site6/web/BO/contract_articles/";
        $this->xml_path="/home/sites/site6/web/BO/clients/";
        $this->sid = session_id();  
    ...
    public function createDeliveryAction()
    {
        ...
        if($this->_request-> isPost())
        {
			...
			{			
				...
				{
                    $delivery_obj->insert();
                    /*added by naseer on 28-10-2015*/
                    $client_obj=new Ep_User_Client();
                    $clientDetails = $client_obj->getClientName($delivery_obj->client_id);
                    $clientName = ($clientDetails[0]['first_name'] != '') ? "<span style=\"text-transform: capitalize;\">".$clientDetails[0]['first_name']." ".$clientDetails[0]['u.last_name']."</span>" : $clientDetails[0]['email'];
                    $user_obj=new Ep_User_User();
                    $login = $user_obj->getBOuser($this->adminLogin->userId);
                    $bo_user_name = $user_obj->getUsername($this->adminLogin->userId);
                    $mail_from = "alert@edit-place.com";
                    //$to_email = "rakeshm@edit-place.com";
                    //$to_email = "kavithashree.r@gmail.com";
                    $to_email = "nass0069@gmail.com";
                    $subject = "Boursorama : New delivery : ".$delivery_obj->title;
                    $message = "Hi Rakesh<br /><br />
                               ".$login." has created delivery titled \"".$delivery_obj->title."\" on ".date('d-m-Y',strtotime($delivery_obj->created_at))." for the client ".$clientName."<br /><br />
                                Thanks,<br />
                                <span style=\"text-transform: capitalize;\">".$bo_user_name."</span> ";
                    $this->sendEmail($mail_from,$to_email,$subject,$message);
                    /*end of added by naseer on 28-10-2015*/
    ...
    /*added by naseer on 28-10-2015*/
    public function sendEmail($mail_from,$to_email,$subject,$message){
            $mail = new Zend_Mail();
