...
class ProofreadController extends Ep_Controller_Action
{
    ...
    public function extractdataAction()
    {
        ...
        $contribs = $user_obj->getContributorsList();
        foreach($contribs as $key=>$value)
        {
            $marksdetials = $user_obj->extractData($contribs[$key]['identifier']);
            $fulldetials[$key]['identifier'] =  $contribs[$key]['identifier'];
            $fulldetials[$key]['email'] =  $contribs[$key]['email'];
            $fulldetials[$key]['profile_type'] =  $contribs[$key]['profile_type'];
            $fulldetials[$key]['totals1marks'] =  ($marksdetials[0]['s1marks'] != '') ? $marksdetials[0]['s1marks'] : 0 ;
            $fulldetials[$key]['totals2marks'] =  ($marksdetials[1]['s2marks'] != '')? $marksdetials[1]['s2marks'] : 0 ;
            $fulldetials[$key]['totalcrtmarks'] =  ($marksdetials[2]['crtmarks'] != '')? $marksdetials[2]['crtmarks'] : 0 ;
            $fulldetials[$key]['s1marks'] =  ($marksdetials[0]['s1avg'] != '')? $marksdetials[0]['s1avg'] : 0 ;
            $fulldetials[$key]['s2marks'] =  ($marksdetials[1]['s2avg'] != '')? $marksdetials[1]['s2avg'] : 0 ;
            $fulldetials[$key]['crtmarks'] = ($marksdetials[2]['crtavg'] != '')? $marksdetials[2]['crtavg'] : 0 ;
            $fulldetials[$key]['s1comt'] =  ($marksdetials[0]['s1comt'] != '')? strip_tags($marksdetials[0]['s1comt']) : 0 ;
            $fulldetials[$key]['s2comt'] =  ($marksdetials[1]['s2comt'] != '')? strip_tags($marksdetials[1]['s2comt']) : 0 ;
            $fulldetials[$key]['crtcomt'] =  ($marksdetials[2]['crtcomt'] != '')? strip_tags($marksdetials[2]['crtcomt']) : 0 ;
            $fulldetials[$key]['s1total'] =  ($marksdetials[0]['s1total'] != '')? $marksdetials[0]['s1total'] : 0 ;
            $fulldetials[$key]['s2total'] =  ($marksdetials[1]['s2total'] != '')? $marksdetials[1]['s2total'] : 0 ;
            $fulldetials[$key]['crttotal'] =  ($marksdetials[2]['crttotal'] != '')? $marksdetials[2]['crttotal'] : 0 ;

        }
        $details = '<table border="1"><tr><td>Identifier</td><td>Email</td><td>Type</td><td>Total S1</td><td>S1 records</td><td>S1 avg</td><td>S1 comments</td><td>Total S2</td><td>S2 records</td><td>S2 avg</td><td>S2 comments</td><td>Total Corrector</td><td>CRT records</td><td>Crt avg</td><td>Crt comments</td></tr>';

        foreach($fulldetials as $key=>$value)
        {
            if($fulldetials[$key]["totals1marks"] != 0 || $fulldetials[$key]["totals2marks"] != 0 || $fulldetials[$key]["totalcrtmarks"] != 0)
            {
                $details.=  '<tr><td>'.$fulldetials[$key]["identifier"].'</td>';
                $details.=   '<td>'.$fulldetials[$key]["email"].'</td>';
                $details.=   '<td>'.$fulldetials[$key]["profile_type"].'</td>';
                $details.=   '<td>'.$fulldetials[$key]["totals1marks"].'</td>';
                $details.=   '<td>'.$fulldetials[$key]["s1total"].'</td>';
                $details.=   '<td>'.$fulldetials[$key]["s1marks"].'</td>';
                $details.=   '<td>'.$fulldetials[$key]["s1comt"].'</td>';
                $details.=   ' <td>'.$fulldetials[$key]["totals2marks"].'</td>';
                $details.=  ' <td>'.$fulldetials[$key]["s2total"].'</td>';
                $details.=   ' <td>'.$fulldetials[$key]["s2marks"].'</td>';
                $details.=   '<td>'.$fulldetials[$key]["s2comt"].'</td>';
                $details.=   ' <td>'.$fulldetials[$key]["totalcrtmarks"].'</td>';
                $details.=   ' <td>'.$fulldetials[$key]["crttotal"].'</td>';
                $details.=   ' <td>'.$fulldetials[$key]["crtmarks"].'</td>';
                $details.=   ' <td>'.$fulldetials[$key]["crtcomt"].'</td></tr>';
            }
        }
    ...
    public function downloadfileAction()
    {
        ...
        if(isset($spec_path))
        {
            ...
            //$oldserver_path = "/home/sites/site5/web/FO/articles/";
            $specpath = ROOT_PATH."FO/client_spec".$details[0]['filepath'];
            $oldspecpath = "/home/sites/site5/web/FO/client_spec".$details[0]['filepath'];
        ...
        $hotelsblwlpath = $this->_request->getParam('hotelsblwlpath');
        if(isset($hotelsblwlpath))
        {
            $details= $artProcess_obj->getArticlePath($hotelsblwlpath);
            $pathId = $details[0]['whitelist_newkeywords'];
            if($pathId)
            {
                $arr = explode("/",$pathId);
                $artId = $arr[0];
                ...
                $fileName = "rapport-erreurs-redacteur-hotels-com-".$language;
                $oldserver_path = "/home/sites/site5/web/articles/";
                $server_path = "/home/sites/site5/web/articles/";
                $dwfile = $server_path.$pathId;
                $olddwfile= $oldserver_path.$pathId;
                if(file_exists($dwfile))
                {
    ...
    /* **** function/code/query optimized on 22.03.2016 **** */
...
//                $res[$key1]['not_plagprocessed']  = $participate_obj->getCountOfNotProcessed($res[$key1]['delivery_id']);
                $totalnotprocessed+= $res[$key1]['not_plagprocessed'];
            }
        ...
        $this->_view->totalnotprocessed = $totalnotprocessed;
    ...
    // when participations refused and mail content displayed in popup //
    public function refusealertAction()
    {
        ...
        if($UserDetails[0]['profile_type'] == 'sub-junior')
        {
            ...
            if($resubtime <= '60')
                ...
                $resubtime= $this->minutesToHours($resubtime)." heures";
    ...
    ////////////////article under stage 0 correction (plagiarism process) ////////////////////////
    ...
    public function stagetestCorrectionAction()
    {
        $parms = $this->_request->getParams();
        print_r($parms); exit;
    }
    ////////////////article under stage 1 correction page ////////////////////////
    public function stage1CorrectionAction()
    {
        ...
        {
            /*if($s1art_params['marks'] == '0')
                $s1art_params['marks'] = NULL;*/
    ...
    public function stage2CorrectionAction()
    {
        ...
        if(isset($s2art_params["s2art_approve"]) && $s2art_params["s2art_approve"]=='yes')//from s2 correction and writerarts pages/
        {
            ...
            $recentversion= $artProcess_obj->getRecentVersion($partId);// print_r($recentversion); exit;
    ...
    public function clientRejectedArtsCorrectionAction()
    {
        ...
        if($artId!=NULL)
        {
            ...
            $partId = $partsOfArt[0]['id'];
            $this->_view->partId = $partId;
            $this->_view->refusecomment = $partsOfArt[0]['refusecomment'];
            $versions_details = $artProcess_obj->getVersionDetails($partId);
            $this->_view->versions_details = $versions_details;
            ////getting the refuse count to change the button refuse to close//////
            $refused_count = $participate_obj->getRefusedCount($partId);
        ...
        if(isset($clientreject_params["clientrejectart_approve"]))
        {
            ...
            if($details[0]['client_validated']=="refuse")
            {
                //Update Article
                $dataArt = array("client_validated"=>NULL);////////updating
                $queryArt = "id= '".$artId."'";
                $article_obj->updateArticle($dataArt,$queryArt);

                //History
                $actparams['artId'] = $artId;
                $actparams['stage'] = "client_rejected";
                $actparams['action'] = "uploadnew";
                $this->articleHistory(60,$actparams);

                //Mail client
                $email=$client_obj->getClientName($details[0]['validated_by']);
                $mail = new Zend_Mail();
                $mail->addHeader('Reply-To','support@edit-place.com');
                $content='Article '.$details[0]['title'].' has been approved';
                $mail->setBodyHtml($content)
                    ->setFrom('support@edit-place.com','Support Edit-place')
                    //->addTo('kavithashree.r@gmail.com')
                    ->addTo($email[0]['email'])
                    ->setSubject('Client rejected article approved');
                $mail->send();
            }
            ...
            if($clientreject_params["clientmail"]!="")
                $autoEmails->sendMailEpMailBox($receiverId,$Object,$Message);
            /// unlock the article///////////////
            $this->unlockonactionAction($artId);
            $this->_helper->FlashMessenger(utf8_decode('Article sent succès'));
            //$this->_redirect("/proofread/stage-articles?submenuId=ML3-SL2&aoId=".$aoId);
    ...
    public function disapproveAction()
    {
        ...
        else if(isset($disapprove_params['pop_disapproves2'])  || isset($disapprove_params['pop_disapproves2_permanent']))
        {
            ...
            $getlatestReason = $artReassign_obj->getLatestReason($partId);
            $reasons = explode(",",$getlatestReason[0]['reasons']);
            $template_obj = new Ep_Message_Template(); //print_r($reasons); //echo count($reasons); exit;
            if(count($reasons) > 1){
                foreach($reasons as $reason)
                {
                    $getReason = $template_obj->getEmailTempDetails($reason);
    ...
    // when somebody wants to open lock the article in stages a intimation mail is sent to article incharge //
    ...
    public function showkeyworddensityAction()
    {
        ...
        $filename = "/home/sites/site6/web/BO/documents/stopwords.txt";
        //chmod($filename,777);
    ...
    {
        $mailParams=$this->_request->getParams();

        if($mailParams['mail_sub'] == 'yes')
        {
            if($this->configval['mail_from'] == '')
                $mail_from = "support@edit-place.com";
            else
                $mail_from = $this->configval['mail_from'];
            if($this->configval['critsend'] == 'yes')
            {
                critsendMail($mail_from, "akeslassy@edit-place.com", stripslashes($mailParams['object']), stripslashes($mailParams['comment']));
            }
            else
            {
                $mail = new Zend_Mail();
                $mail->addHeader('Reply-To',$mail_from);
                $mail->setBodyHtml(stripslashes($mailParams['comment']))
                    ->setFrom($mail_from)
                    ->addTo("akeslassy@edit-place.com")
                    ->setSubject(stripslashes($mailParams['object']));
                $mail->send();
            }
            exit;
        }
        else
            $this->_view->render("proofread_notemplate");
    }
    ...
    // the count or rate of contributro being participated and selected by BO //
    ...
    // tracking down the actions performed on the Article ///
    ...
    // loading the published artcles via ajax  //
    ...
    public function downloadarticleAction()
    {
        ...
        {
            $details= $article_obj->getArticleDetails($spec_path);
            $specpath="/home/sites/site5/web/FO/client_spec".$details[0]['filepath'];
            $fileName = $details[0]['deliveryTitle']."-specs";
            if(!is_dir($specpath))
            {
                $attachment=new Ep_Message_Attachment();
    ...
    // validating the article in stage1 in bulk //
    ...
    // validating the article in stage2 in bulk //
    public function bulkvalidatestage2artsAction()
    {
        ...
        //echo "<pre>";print_r($validateDetails);exit; //exit('partId='.$partId);
        ...
        foreach($validateDetails as $validateDetail) :
            ...
            $artId = $validateArt_[0];
            $partId = $validateArt_[1];
            $details = $article_obj->getArticleDetails($artId);     //print_r($details);
    ...
    // validating the article from the stage 1 correction page to skip the s2 correction page //
    ...
    public function downloadclientfileAction()
    {
        ////////download //////////////////////////////////////////
        $prevurl = getenv("HTTP_REFERER");
        $part_obj = new EP_Participation_Participation();
        $ParticipationDetails=$part_obj->getParticipateDetails($_REQUEST['partid']);

        $filepath = "/home/sites/site5/web/FO/sc_mail/".$ParticipationDetails[0]['clientcommentfile'];

        if(file_exists($filepath))
        {
            $attachment=new Ep_Message_Attachment();
            $attachment->downloadAttachment($filepath,'attachment', $ParticipationDetails[0]['clientcommentfile']);
            exit;
        }
        else
        {
        ...
        if(isset($s2art_params["s2art_approve"]) && $s2art_params["s2art_approve"]=='yes')//from s2 correction and writerarts pages/
        {
            ...
            if($_FILES['art_doc_'.$partId]['tmp_name'] != '')
            {
                ...
                /*added by naseer  */
                    ...
                    {
                        $stencil_data = array("stencil_content"=>$s2art_params['stencil_text'][$k],"article_id"=>$artId,"delivery_id"=>$aoId);
                ...
                $data = array("marks"=>$s2art_params["markstotal"], "reasons_marks"=>$s2art_params["markswithreason"], "comments"=>$s2art_params["marks_comments"],"article_doc_content"=>utf8_encode($article_doc_content),"article_path"=>$article_path,"article_name"=>$article_name);////////updating
                $query = "participate_id= '".$partId."' AND version='".$recentversion[0]['version']."'";
                $artProcess_obj->updateArticleProcess($data,$query);
                /////udate status article process table///////
                /* $data = array("status"=>"approved", "stage"=>'s2');////////updating
