...
class RepublishController extends Ep_Controller_Action {
    ...
    public function closeartprofileAction()
    {
        ...
        else
        {
			...
			//simultaneous correction mail for correctors when no participation
			$delivery_details=$delivery_obj->getArtDeliveryDetails($artId);
			if($delivery_details[0]['articlecorrection']=="yes")
			{
				$selectedcorrector=$crtparticipate_obj->getSelectedCorrector($artId);
				...
				if($selectedcorrector!="NO")
				{
					...
					$automail->messageToEPMail($selectedcorrector,187,$parameters);
				}
			}
    ...
    public function getclosemailpopupAction()
    {
        ...
        {
            $mailid = 27;
            $popuphead = $text = "Voulez-vous r&eacute;ellement fermer cet article? Il y a eu ".$usercount." participations, voici le mail qui leur sera envoy&eacute;:";
        }
    ...
    public function republishpopupAction()
    {
        ...
        $artdeldetails[0]['publang'] = explode(",",$artdeldetails[0]['publang']);
        /*foreach($pub_lang AS $key=>$val)
        {
            $publish_lang[$val] = $this->language_array[$val];
        }
        $this->_view->publish_language = $publish_lang;*/
        if($republishParams['save'] == 'save')   // request from the ajax request //
    ...
    /// loading the users based on the article particiapations based on ajax call for the drop down value selected ///
    ...
    /// loading the bulk users based on the article particiapations based on ajax call for the drop down value selected ///
    ...
    // ajax call for the users of public aos  ///
    ...
    // get the contributors who already participated in article to send the contributor ///
    ...
    public function publisharticlefoAction()
    {
        ...
        //Simultaneous correction mails for corrector
		if($artdeldetails[0]['articlecorrection']=="yes")
		{
			$selectedcorrector=$corr_obj->getSelectedCorrector($artId);
			
			if($selectedcorrector!="NO")
			{
				$parameters['articlename_link']='/contrib/ongoing';
				$parameters['article_title']=$artdeldetails[0]['articleName'];
					$max=max($artdeldetails[0]['st'], $artdeldetails[0]['jt'], $artdeldetails[0]['sjt']);
					$writersubmission=time()+(60*$max);
					$writerparticipation=60*$artdeldetails[0]['articleparttime'];
				$parameters['max_reception_writer_file_date_hour']=strftime("%d/%m/%Y &agrave; %H:%M",($writerparticipation+$writersubmission)); 
					
				
				$timeoutpart=$participate_obj->checkRecordPresent($artId,'time_out','contributor');
				if($timeoutpart=="YES")
					$autoEmails->messageToEPMail($selectedcorrector,184,$parameters);
				
				$partcount=$participate_obj->getParticipationCount($artId);
				if($partcount==0)
					$autoEmails->messageToEPMail($selectedcorrector,185,$parameters);
			}
		}
		//exit;
		$partdetails =  $participate_obj->getParticipantsDetailsCycle0($artId);
