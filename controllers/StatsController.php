...
class StatsController extends Ep_Controller_Action
{
    ...
    public function generatepdfAction()
    {
		    ...
		    $client_id= $details['client_id'];
                    ...
                    $invoice_details_pdf.='<tr>
                                            <td colspan="2">'.$details['AOTitle'].' - '.$client_name.' - '.$article_created_date.'</td>
               ...
               /**Total Invoice*/
                ...
                if($invoiceDetails[0]['payment_info_type']=='ssn')
                {
                    ...
                    $tax_details_pdf='<tr>
                                        ...
                                        <td width="33%" >'.number_format((($total*0.85)/100),2,',','').' &#x80;</td>
                                        </tr>
                                        <tr>
                                        <td>CSG</td>
                                        <td>taux : 7,36875%</td>
                                        <td >'.number_format((($total*7.36875)/100),2,',','').' &#x80;</td>
                                        </tr>
                                        <tr>
                                        <td>CRDS</td>
                                        <td>taux : 0,49125% </td>
                                        <td >'.number_format((($total*0.49125)/100),2,',','').' &#x80;</td>
                                        </tr>';
                    ...
                    if($tax_date >= date("Y-m-d",strtotime('2012-06-22')))
                    {
                         $tax_details_pdf.='<tr>
                                            ...
                                            <td>taux : 0,35% </td>
                                            <td>'.number_format((($total*0.35)/100),2,',','').' &#x80;</td>
                                            </tr>';
                    }
                ...
                else if($invoiceDetails[0]['payment_info_type']=='comp_num' && $invoiceDetails[0]['vat_check']=='YES' )
                {
				...
				else if($invoiceDetails[0]['payment_info_type']=='ep_admin') //Added w.r.t new admin fees
                  {
                      ...
                      $tax_details_pdf='                                            
                            ...
                            <td colspan="2">Frais de traitement administratif Edit-place : '.$admin_fee_percentage.'%</td>
                            
                            <td>'.number_format((($total*$admin_fee_percentage)/100),2,',','').' &#x80;</td>
                                              </tr>                                            
                                              ';
                  }
                     ...
                     if($period_month>0)
                      ...
                      {                          
                       ...
                       $period_fees=number_format(((($total-$totalTax)*$fees_percentage)/100),2,'.',''); 
                        ...
                        $tax_details_pdf.='
						<tr class="alert alert-danger">
                            <td colspan="2">Frais d\'avance : '.$fees_percentage.'% (paiement le 15 '.$fees_paid_month.' )</td>
                            
                            <td>'.number_format($period_fees,2,',','').' &#x80;</td>
                                              </tr>'; 
               ...
               $final_invoice_amount='<table class="change_order_items table" width="100%"><tr class="alert alert-info">
                                        <td style="text-align:right;" colspan="2"><h4>Montant &agrave; verser &agrave; l\'auteur</h4></td>
                                        <td><h4>'.number_format($this->_view->FinaltotalInvoice,2,',','').' &#x80;</h4></td>
                                        </tr></table>';
            }
            /**Wire OR paypal info**/
                ...
                $bank_transfer_price='';
                       ...
                       else if($invoiceDetails[0]['payment_type']=="virement")
                       {
							...
							else
							{
                                ...
                                $bank_account_name="Nom du b&eacute;n&eacute;ficiaire : ".$invoiceDetails[0]['bank_account_name']."<br>";
   ...
   /*generating article titles*/
    ...
    public function downloadinvoiceAction()
    {
        ...
        {
            $invoicePDFPath="/home/sites/site5/web/FO/invoice/".$invoice_path;
    ...
    public function paidInvoicesAction()
    {
        ...
        if($_REQUEST['debug']){//echo '<pre>';print_r($res);
        exit;}
        if($res!="NO")
        {
            $this->_view->paginator = $res;
                ...
                for ( $j=0 ; $j<count($aColumns) ; $j++ )
                {
                    ...
                    else
                    {
                        ...
                        elseif($aColumns[$j] == 'total_invoice_paid')
                            $row[] = '<span class="label label-inverse pull-right">'.$rResult[$i]["total_invoice_paid"].'</span>';
