...
class AoController extends Ep_Controller_Action
{
    ...
    { 
		...
		//to fetch all invoiced clients from db
       ...
       $this->_view->contrib_array=array();
		...
		if($this->_view->nav_1==1)
		{
			...
			$this->_view->estimated_worktime=$this->AO_creation->ao_step1['estimated_worktime'];
			$this->_view->estimated_worktime_min=$this->AO_creation->ao_step1['estimated_worktime_min'];
			$this->_view->estimated_worktime_option=$this->AO_creation->ao_step1['estimated_worktime_option'];
			...
			//Correction Cost Set For Correction type interne Or Freelance 
			if($this->AO_creation->ao_step1['correction']=="intern" || $this->AO_creation->ao_step1['correction']=="freelance")
			{
					$this->_view->correction_cost=$this->AO_creation->ao_step1['correction_cost'];
			}
		...
		else
		{
			...
			/*$this->_view->junior_time=$this->configval["jc_time"]/60;
			$this->_view->senior_time=$this->configval["sc_time"]/60;
			$this->_view->subjunior_time=$this->configval["jc0_time"]/60;
			$this->_view->jc_resubmission=$this->configval["jc_resubmission"]/60;
			$this->_view->sc_resubmission=$this->configval["sc_resubmission"]/60;
			$this->_view->jc0_resubmission=$this->configval["jc0_resubmission"]/60;*/
			...
			//$this->_view->price_min='Min.';
			//$this->_view->price_max='Max.';
			
			$this->_view->correction_participation=$this->configval['correction_participation']/60;
			/*$this->_view->correction_jc_submission=$this->configval['correction_jc_submission']/60;
			$this->_view->correction_sc_submission=$this->configval['correction_sc_submission']/60;
			$this->_view->correction_jc_resubmission=$this->configval['correction_jc_resubmission']/60;
			$this->_view->correction_sc_resubmission=$this->configval['correction_sc_resubmission']/60;*/
			...
			$this->_view->pricedisplay='no';
			$this->_view->estimated_worktime_option='hour';
			$this->_view->estimated_worktime='';
			$this->_view->estimated_worktime_min='';
			//$this->_view->publish_langarray=array_keys($contrib_lang_array);
    ...
    public function aoCreate2Action()
    {
		...
		$this->_view->Contrib_langs = $contrib_lang_array;
		$correction_cost=array();
		
		//Private contribs
		if($this->AO_creation->ao_step1['testrequired']=='yes')
			$contriblistall = $del_obj->getAllContribArticleTest($this->AO_creation->ao_step1['contribtype'],$this->AO_creation->ao_step1['contrib_cat'],$this->AO_creation->ao_step1['contrib_lang'],$this->AO_creation->ao_step1['testrequired'],$this->AO_creation->ao_step1['testmarks']);
		else
			$contriblistall=$del_obj->getAllContribArticle($this->AO_creation->ao_step1['contribtype'],$this->AO_creation->ao_step1['contrib_cat'],$this->AO_creation->ao_step1['contrib_lang']);
			
		$this->_view->contrib_array=array();
		$contriblistall1=array();
		$correction_pricemax=array();
		$correction_pricemin=array();
		$correction_cost=array();
			for ($i=0;$i<count($contriblistall);$i++)
			{
				$contriblistall1[]=$contriblistall[$i];
		...
		if($this->_view->nav_2==1)
		{
			...
			{	
				...
				if($this->AO_creation->ao_step1['AOtype']=='private')
				{
					...
					$contrib_array[]=explode(",",$this->AO_creation->ao_step2['favcontribchecklist'][$a]);
				}
				...
				//correction code
				$correction_orig=$this->AO_creation->ao_step1['correction'];
				if($this->AO_creation->ao_step1['correction']=='on'){
					//$correction[]=$this->AO_creation->ao_step2['correction_'.($a+1)];				
					$correction[]='on';				
					if($this->AO_creation->ao_step2['correction_'.($a+1)]=='on')
					{	
						//echo 'here';
					...
					if($this->AO_creation->ao_step2['correction_'.($a+1)]=='intern' || $this->AO_creation->ao_step2['correction_'.($a+1)]=='freelance'){
						$correction_cost[]=$this->AO_creation->ao_step2['correction_cost'][$a];
					}else{
						$correction_cost[]='';
					}
					...
					if($this->AO_creation->ao_step1['correction_type']=='private')
					{
						...
						if(count($corrector_array)==0)
							$corrector_array[]=$this->AO_creation->ao_step1['correctorcheck'];
					}
					
				}else{
					$correction[]=($this->AO_creation->ao_step2['correction_'.($a+1)]!='on') ? $this->AO_creation->ao_step2['correction_'.($a+1)] : $this->AO_creation->ao_step1['correction'];
					if($this->AO_creation->ao_step2['correction_'.($a+1)]=='intern' || $this->AO_creation->ao_step2['correction_'.($a+1)]=='freelance'){
						$correction_cost[]=$this->AO_creation->ao_step2['correction_cost'][$a];
					}else{
						if($this->AO_creation->ao_step1['correction_cost']!=$this->AO_creation->ao_step2['correction_cost'][$a]){
							$correction_cost[]=$this->AO_creation->ao_step1['correction_cost'];
						}else{
							$correction_cost[]=$this->AO_creation->ao_step2['correction_cost'][$a];
						}
			...
			{	
				...
				for($d=0;$d<$difftot;$d++)
				{
					...
					$correction[]=$this->AO_creation->ao_step1['correction'];
					$correction_orig=$this->AO_creation->ao_step1['correction'];
					if($this->AO_creation->ao_step1['correction']=='on')
					{
		...
		else
		{
			...
			for($a=0;$a<$tot;$a++)
			{
			  ...
			  $language[][0]=$this->AO_creation->ao_step1['language'];
		...
		$this->_view->aotype=$this->AO_creation->ao_step1['AOtype'];
		$this->_view->correction=$correction;//$this->AO_creation->ao_step1['correction'];
		$this->_view->correction_orig=$correction_orig;
		$this->_view->correction_type=$this->AO_creation->ao_step1['correction_type'];//$this->AO_creation->ao_step1['correction_cost'];
		$this->_view->correction_cost=$correction_cost;//$this->AO_creation->ao_step1['correction_cost'];
		$this->_view->mission_type=$this->AO_creation->ao_step1['mission_type'];
    ...
    public function aoCreate2lotAction()
    {
		...
		//Private contribs
		/* If the mission requires tested contributors, else in normal condition*/
		if($this->AO_creation->ao_step1['testrequired']=='yes')
			$contriblistall = $del_obj->getAllContribArticleTest($this->AO_creation->ao_step1['contribtype'],$this->AO_creation->ao_step1['contrib_cat'],$this->AO_creation->ao_step1['contrib_lang'],$this->AO_creation->ao_step1['testrequired'],$this->AO_creation->ao_step1['testmarks']);
		else
			$contriblistall=$del_obj->getAllContribArticle($this->AO_creation->ao_step1['contribtype'],$this->AO_creation->ao_step1['contrib_cat'],$this->AO_creation->ao_step1['contrib_lang']);
       ...
       // echo "<pre>";print_r($correction_cost);exit;
		$this->_view->totalart=$tot; 
		$this->_view->aotype=$this->AO_creation->ao_step1['AOtype'];
		$this->_view->correction=$correction;//$this->AO_creation->ao_step1['correction'];
		$this->_view->correction_orig=$correction_orig;
		$this->_view->correction_type=$this->AO_creation->ao_step1['correction_type'];
		$this->_view->correction_cost=$correction_cost;//$this->AO_creation->ao_step2['correction_orig'];
		$this->_view->mission_type=$this->AO_creation->ao_step1['mission_type'];
		$this->_view->whitelist=$this->AO_creation->ao_step1['white_list'];
		$this->_view->blacklist=$this->AO_creation->ao_step1['black_list'];
        $this->_view->blwlcheck=$this->AO_creation->ao_step1['blwl_check'];
    ...
    {	
		$this->AO_creation = Zend_Registry::get('AO_creation');
		$params2=$this->_request->getParams();//echo "<pre>";print_r($params2);echo "</pre>"; 
		//print_r(explode(",",$params2['favcontribchecklist_1']));
    ...
    public function aoCreate4Action()
    {
		...
		elseif($this->AO_creation->ao_step1['correction']=="intern")
				$darray["correction"]="internal";
		elseif($this->AO_creation->ao_step1['correction']=="freelance")
				$darray["correction"]="freelancer";
		...
		$darray["mailsubject"]=utf8_decode($this->AO_creation->ao_step3['mailsubject']);
		...
		if($this->AO_creation->ao_step1['missiontest']=="on")
		{	
			$this->AO_creation->ao_step2['missiontest']=="on";
		...
		//print_r($this->AO_creation->ao_step1);	
		//print_r($darray);exit;
		...
		//If delivery insertion is done properly
			if($delivery!="false")
			{
				...
				//language  in multi language case
				$this->AO_creation->ao_step2['step1language']=$this->AO_creation->ao_step1['language'];
				...
				//$this->AO_creation->ao_step2['correction_cost'] = $this->AO_creation->ao_step1['correction_cost'];
				...
				$this->AO_creation->ao_step2["estimated_worktime"] = $this->AO_creation->ao_step1['estimated_worktime'];
				$this->AO_creation->ao_step2["estimated_worktime_min"] = $this->AO_creation->ao_step1['estimated_worktime_min'];
				$this->AO_creation->ao_step2["estimated_worktime_option"] = $this->AO_creation->ao_step1["estimated_worktime_option"];
							...
							if($aoDetails[0]['AOtype']=='private')
							{
								...
								if(is_array($contributors) && count($contributors)>0)
								{
									...
									if($aoDetails[0]['premium_option']=='0'){
										...
										{
											$farray=explode(",",$this->AO_creation->ao_step2['favcontribchecklist'][$t]);
											if(count($farray)>1)
												$single='no';
										}
										if($single=='yes')
	...
	public function updatequizlistAction()
	{
		...
		$quizlist.='<select name="quiz" id="quiz" onChange="loadquizvals();"><option value="">S&eacute;lectionner</option>';
	...
	public function updatecontriblistAction()
	{
		...
		//if, when tested contributors are required, otherwise else
		if($_REQUEST['test']=='yes')
			$contriblistall = $del_obj->getContribsByTypeTest($_REQUEST['type'],$_REQUEST['category'],$_REQUEST['language'],$_REQUEST['test'],$_REQUEST['testmarks']);
		else
			$contriblistall = $del_obj->getContribsByType($_REQUEST['type'],$_REQUEST['category'],$_REQUEST['language']);
		//print_r($contriblistall);
		$contriblistall1=array();
			for ($i=0;$i<count($contriblistall);$i++)
				{
					$contriblistall1[]=$contriblistall[$i];
	...
	public function updatecorrectorlistAction()
	{
		...
		$contriblist.='<select multiple="multiple" name="correctorcheck[]" id="correctorcheck">';
			...
			if(count($this->AO_creation->ao_step1['correctorcheck'])>0 && $_REQUEST['parameter']==1)	
			{
				foreach ($contriblistall1 as $contrib)
				{
	...
	/* Ajax call to update private corrector list in step2 onchange of article language*/
	public function updatecorrectorlistarticleAction()
	{
		$this->AO_creation = Zend_Registry::get('AO_creation');
		...
		$contriblist.='<select multiple="multiple" name="correctorcheck['.$index.'][]" id="correctorcheck_'.$_REQUEST['index'].'" class="favcontribcheck">';
				foreach ($contriblistall1 as $contrib)  
				{
					$contriblist.='<option value="'.$contrib['identifier'].'" selected>'.utf8_encode($contrib['name']).'</option>';
				}
		...
		$contriblist.='</select>
			<div id="corrcontriberr_'.$_REQUEST['index'].'" style="color:red;"></div>';
			
		echo $contriblist;   
	}
	
	/* Ajax call to update private contributors list in step1 onchange of language & category */
	public function updatecontriblistarticleAction()
	{
		...
		{	//if step2 session exists
			$checklist=explode(",",$this->AO_creation->ao_step2['favcontribchecklist'][$index]);
			foreach ($contriblistall1 as $contrib)
			{
				if(in_array($contrib['identifier'],$checklist))
					$contriblist.='<option value="'.$contrib['identifier'].'" selected>'.utf8_encode($contrib['name']).'</option>';
				else
					$contriblist.='<option value="'.$contrib['identifier'].'">'.utf8_encode($contrib['name']).'</option>';
			}
		...
		{	//if step2 session does not exists & coming from step1 to step2
			foreach ($contriblistall1 as $contrib)
			{
				if(in_array($contrib['identifier'],$this->AO_creation->ao_step1['favcontribcheck']))
					$contriblist.='<option value="'.$contrib['identifier'].'" selected>'.utf8_encode($contrib['name']).'</option>';
		...
		{   //on change of language , category
			foreach ($contriblistall1 as $contrib)
			{
				$contriblist.='<option value="'.$contrib['identifier'].'" selected>'.utf8_encode($contrib['name']).'</option>';
			}
		...
		$contriblist.='</select>
			<input type="hidden" name="favcontribchecklist['.$index.']" id="favcontribchecklist_'.$_REQUEST['index'].'" value="" />
	...
	public function aolinkpollAction()
	{
		...
		if($_REQUEST['client']!="") //Loading Polls
		{
			...
			$content.='<select name="pollao" id="pollao" onChange="loadpollcontent();" style="width:250px;">';
				$content.='<option value="">S&eacute;lectionner</option>';
		...
		elseif($_REQUEST['poll']!="") // Loading other data on selection of Poll
		{
			...
			$content.='<div style="min-height:auto;max-height:200px;overflow:auto;margin-right:6px;overflow-x:hidden">';
				...
				{
					$file = "/home/sites/site5/web/FO/profiles/contrib/pictures/".$poll_contrib[$pc]['user_id']."/".$poll_contrib[$pc]['user_id']."_h.jpg";
						if(file_exists($file))
	...
	public function uploadaospecAction()
	{
		...
		$uploaddir1 = '/home/sites/site5/web/FO/client_spec/';
		...
		$file1 = $uploaddir1 . $bname;
		if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file))
		{
			chmod($file,0777);
			move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file1);
			chmod($file1,0777);
			echo "success#".$bname;
		}
		else
	...
	public function getcontribmailcontentAction()
	{
		...
		$aowithlink='<a href="http://ep-test.edit-place.com/contrib/aosearch">'.stripslashes(utf8_encode($this->AO_creation->ao_step1['title'])).'</a>';
		$sub=utf8_encode($email[0]['Object']);
		$Message=utf8_encode($email[0]['Message']);
            ...
            if($ticket->insert())
               {
					...
					{
						//echo $UserDetails[0]['email'].",".$object.",".$text_mail;
						if($senderfrom=='me')
						{
							...
							$content.="<br><br>Vous ne souhaitez plus &ecirc;tre alert&eacute; ? <a href=\"http://ep-test.edit-place.com/user/alert-unsubscribe?uaction=unsubscribe&user=".MD5('ep_login_'.$email)."\">Cliquez-ici</a>";
							$mailbody=$content;			
						}
						elseif($this->AO_creation->ao_step3['sendfrom']=='editorial')
						{
							...
							$fromURL="https://secure.edit-place.fr/contrib/aosearch";
								$toURL="https://secure.edit-place.fr/user/email-login?user=".MD5('ep_login_'.$email)."&hash=".MD5('ep_login_'.$password)."&type=".$type."&red_to=aosearch";
							$content=str_replace($fromURL,$toURL,$content);
						...
						if($this->configval["critsend"]=="yes")
							critsendMail('support@edit-place.com', $UserDetails[0]['email'], $object, $mailbody);
						else
						{
							$mail = new Zend_Mail();
	...
	public function loadtemplateaoAction()
	{
		...
		if(count($aovalues)>0)
		{
			...
			for($a=0;$a<count($articlevalues);$a++)
			{
				...
				$this->AO_creation->ao_step2['submit_option'][$a]=$articlevalues[$a]['submit_option'];
				if($articlevalues[$a]['submit_option']=="hour")
				{
					$this->AO_creation->ao_step2['subjunior_time'][$a]=$articlevalues[$a]['subjunior_time']/60;
					$this->AO_creation->ao_step2['junior_time'][$a]=$articlevalues[$a]['junior_time']/60;
					$this->AO_creation->ao_step2['senior_time'][$a]=$articlevalues[$a]['senior_time']/60;
				}
				elseif($articlevalues[$a]['submit_option']=="day")
				{
					$this->AO_creation->ao_step2['subjunior_time'][$a]=$articlevalues[$a]['subjunior_time']/(60*24);
					$this->AO_creation->ao_step2['junior_time'][$a]=$articlevalues[$a]['junior_time']/(60*24);
					$this->AO_creation->ao_step2['senior_time'][$a]=$articlevalues[$a]['senior_time']/(60*24);
				}
				else
				{
					$this->AO_creation->ao_step2['subjunior_time'][$a]=$articlevalues[$a]['subjunior_time'];
					$this->AO_creation->ao_step2['junior_time'][$a]=$articlevalues[$a]['junior_time'];
					$this->AO_creation->ao_step2['senior_time'][$a]=$articlevalues[$a]['senior_time'];
				}
				
				$this->AO_creation->ao_step2['resubmit_option'][$a]=$articlevalues[$a]['resubmit_option'];
				if($aovalues[0]['resubmit_option']=="hour")
				{
					$this->AO_creation->ao_step2['jc0_resubmission'][$a]=$articlevalues[$a]['jc0_resubmission']/60;
					$this->AO_creation->ao_step2['jc_resubmission'][$a]=$articlevalues[$a]['jc_resubmission']/60;
					$this->AO_creation->ao_step2['sc_resubmission'][$a]=$articlevalues[$a]['sc_resubmission']/60;
				}
				elseif($aovalues[0]['resubmit_option']=="day")
				{
					$this->AO_creation->ao_step2['jc0_resubmission'][$a]=$articlevalues[$a]['jc0_resubmission']/(60*24);
					$this->AO_creation->ao_step2['jc_resubmission'][$a]=$articlevalues[$a]['jc_resubmission']/(60*24);
					$this->AO_creation->ao_step2['sc_resubmission'][$a]=$articlevalues[$a]['sc_resubmission']/(60*24);
				}
				else
				{
					$this->AO_creation->ao_step2['jc0_resubmission'][$a]=$articlevalues[$a]['jc0_resubmission'];
					$this->AO_creation->ao_step2['jc_resubmission'][$a]=$articlevalues[$a]['jc_resubmission'];
					$this->AO_creation->ao_step2['sc_resubmission'][$a]=$articlevalues[$a]['sc_resubmission'];
				}
				...
				$this->AO_creation->ao_step2['column_xls'][$a]=$articlevalues[$a]['column_xls'];
			}
			*/
			/********************************** STEP 3 *************************************/
	...
	public function clientprofilecheckAction(){
        ...
        if(file_exists('/home/sites/site5/web/FO/profiles/clients/logos/'.$clientParams['client'].'/'.$clientParams['client'].'_global.png') && ($user_detail[0]['company_name'] != ''))
           $send="yes";
        else
            $send="no";
	...
	function checkclientprofileAction()
	{
        ...
        if( file_exists('/home/sites/site5/web/FO/profiles/clients/logos/'.$clientParams['client'].'/'.$clientParams['client'].'_global.png')  )
            exit('yes') ;
        else
            exit('no') ;
    }
	...
	{ 
		...
		$this->_view->page_title="Edit-place Admin : Cr�er sondage";
	...
	public function createpoll3Action()
	{
		...
		$this->Poll_creation->poll_step3=$params4;
			...
			$parameters1['client_polllink']="<a href='http://ep-test.edit-place.com/client/devispremium?id=".$poll_id."'>ici</a>";
	...
	public function uploadpollspecdocAction()
	{
		...
		$realfilename=$_FILES['uploadfile']['name'];
		$ext=$this->findexts($realfilename);
		$uploaddir = '/home/sites/site5/web/FO/poll_spec/';
		//$uploaddir=$uploaddir."/";
		$bname=basename($realfilename,".".$ext)."_".uniqid().".".$ext;
		$file = $uploaddir . $bname;
		if (move_uploaded_file($_FILES['uploadfile']['tmp_name'], $file))
	...
	public function pollAction()
	{
		...
		$this->_view->page_title="Edit-place Admin : Sondage";
	...
	public function pollparticipationstatusAction()
	{
		...
		{
			$actvar="active";
			$data['text']='<b><a href="javascript:void(0);" onClick="pollparticipationactive('.$_REQUEST['partid'].',\''.$actvar.'\');">Exclure</a></b>';
		}
		else
		{
			$actvar="inactive";
			$data['text']='<b><a href="javascript:void(0);" onClick="pollparticipationactive('.$_REQUEST['partid'].',\''.$actvar.'\');">Inclure</a></b>';
		}
		...
		if(count($pollstats)>0)
		{
			foreach($pollstats as $stat)
			{
					...
					{
						$stats.='<tr>
							<td width="30%">Nombre de r&eacute;ponses : </td>
							<td align="left">'.$stat['partcount'].'</td>
							<td align="right">Minimum : </td>
							<td>'.$this->zero_cut($stat['min'],2).' &euro;</td>
							
						</tr>
						<tr>
							<td colspan="2"></td>
							<td align="right">Maximum : </td>
							<td>'.$this->zero_cut($stat['max'],2).' &euro;</td>
						</tr>
						<tr>
							<td colspan="2"></td>
							<td style="font-weight:bold;" align="right">Moyenne : </td>
							<td style="font-weight:bold;">'.$this->zero_cut($stat['avg'],2).' &euro;</td>
						</tr>';
					}
					...
					{
						$stats.='<tr>
							<td align="right" width="30%">Nombre de r&eacute;ponses : </td>
							<td>'.$stat['partcount'].'</td>
							</tr>';
					}
					else
					{
						...
						$stats.='<tr>
							<td align="right" width="30%" valign="top">Nombre de r&eacute;ponses : </td>
	...
	public function pollpopupAction()
	{
		...
		if($_REQUEST['tooltype']=='poll')
		{
			...
			$this->_view->polllist=$poll_obj->ListPollresult($_REQUEST['poll']);
             ...
             {
                $file = "/home/sites/site5/web/FO/profiles/contrib/pictures/".$PollParticipationDetails[$i]['user_id']."/".$PollParticipationDetails[$i]['user_id']."_h.jpg";
                 if(file_exists($file))
                 {
		...
		elseif($_REQUEST['tooltype']=='user')
		{
			$PollCOntributorDetails=$poll_obj->PollContributorDetails($_REQUEST['user']);
				$file = "/home/sites/site5/web/FO/profiles/contrib/pictures/".$PollCOntributorDetails[0]['user_id']."/".$PollCOntributorDetails[0]['user_id']."_h.jpg";
                 if(file_exists($file))
				...
				elseif($PollCOntributorDetails[0]['education']==6)
				  $PollCOntributorDetails[0]['education']='Bac+5 et plus';
										...
										<tr>
											<td>
												<table>
														...
														<tr>
															<td>Cat&eacute;gories Phares</td>
															<td>: <b>'.utf8_encode($PollCOntributorDetails[0]['favourite_category']).'</b></td>
														</tr>
														<tr><td>&nbsp;</td></tr>
														<tr>
															<td colspan="2" valign="top">Texte de pr&eacute;sentation</td>
	...
	public function pollconfigurationAction()
	{
		...
		$pquestionlist=$ques_obj->getPollquestions();
			...
			for($q=0;$q<count($pquestionlist);$q++)
			{
				...
				if($pquestionlist[$q]['type']=='price')
					$pquestionlist[$q]['type']='Prix';
				elseif($pquestionlist[$q]['type']=='timing')
					$pquestionlist[$q]['type']='Dur&eacute;e';
				elseif($pquestionlist[$q]['type']=='radio')
					$pquestionlist[$q]['type']='Boutons radio';
				elseif($pquestionlist[$q]['type']=='bulk_price')
					$pquestionlist[$q]['type']='Prix de gros';
				elseif($pquestionlist[$q]['type']=='range_price')
					$pquestionlist[$q]['type']='Fourchette de prix';
				elseif($pquestionlist[$q]['type']=='checkbox')
					$pquestionlist[$q]['type']='Cases &agrave; cocher';
		...
		$this->_view->pquestionlist=$pquestionlist;
		$this->_view->page_title="Edit-place Admin : Configuration sondage";
	...
	public function configurationAction()
	{
		...
		$this->_view->page_title="Edit-place Admin : Configuration";
    ...
    {	
		...
		$this->_view->now=strtotime("now");
		$this->_view->page_title="Edit-place Admin : Missions libert� en cours";
    ...
    public function overDueAction()
    {
		...
		$this->_view->page_title="Edit-place Admin : Articles en retard";
        if($articleParams['action_from']=='popup')
    ...
    public function getMailComments($mailid,$parameters)
    {
		...
		$ongoinglink='<a href="http://ep-test.edit-place.com'.$parameters['ongoinglink'].'">cliquant ici</a>';
	...
	public function getextendmailAction()
    {
        ...
        if(!$articleParams['extend_time'])
        {
           $articleParams['extend_time']=0;
			...
			if($articleParams['usertype']=='corrector')
			$emailContent=str_replace(htmlentities('r�diger'),"corriger",$emailContent);
	...
	public function premiumoptionsAction()
    {
        ...
        $this->_view->page_title="Edit-place Admin : Option haut de gamme";
	...
	public function categorypriceAction()
	{
		...
		$this->_view->pagelimit=$this->getConfiguredval('pagination_bo');
		$this->_view->page_title="Edit-place Admin : Category Price";
	...
	public function automatizescjcAction()
	{
		...
		$this->_view->page_title="Edit-place Admin : Automatize JC & SC";
	...
	/* Upgrading/downgrading writer profile type from Mission test stats page */
	...
	/* Function to upload and read csv data in AO creation step2, to fill out article titles dynamically*/
	public function uploadartcsvAction()
	{
		//print_r($_FILES);exit;
		$realfilename=$_FILES['uploadfile']['name'];
		$ext=$this->findexts($realfilename);
		$uploaddir = '/home/sites/site5/web/FO/test_csv/';
	...
	/*public function googletranslateAction()
	{
		...
		$apiKey = 'AIzaSyAXPjgqbk_zycyV4bW7B0o4FfvTMK1NNYE';
	...
	/* Function to generate and download client invoice pdf */
	public function downloadinvoicetestAction()
    {
        //Ao id
		$invoiceid= $_REQUEST['delivery']; 
		
		//Path where invoice file to be stored
       ...
       // {
            ...
            //Payment details
            $payment=$pay_obj->getdetailsDelivery($invoiceid);
			...
			$invoice_details_pdf.='<tr>
                                            <td>'.$payment[$a]['title'].'</td>
                                            <td class="change_order_total_col">'.number_format($payment[$a]['price_user'],2,',','').'</td>
											<td class="change_order_total_col">'.number_format($payment[$a]['price_user'],2,',','').'</td>
                                            </tr>';
										$total+=$payment[$a]['price_user'];
			}
			$ep=0.65*$total;
            $invoice_details_pdf.='<tr>
            ...
            $payinfo_number="";
			$total+=$ep;
            if($payment[0]['client_type']!="personal")
            {
                //Tax details
	...
	/* Function to generate and download client invoice pdf */
	public function downloadinvoicetest1Action()
    {
        $invoiceid= $_REQUEST['delivery']; 
        $invoicedir='/home/sites/site5/web/FO/invoice/client/'.$_REQUEST['client'].'/';
       ...
       // {
            ...
            for($a=0;$a<count($payment);$a++)
			{
			$epprice=$payment[$a]['price_user'] * ((100-$payment[$a]['contrib_percentage'])/100);
			$articleprice=$payment[$a]['price_user'] + $epprice;
			$invoice_details_pdf.='<tr>
                                            <td>'.$payment[$a]['title'].'</td>
                                            <td class="change_order_total_col">'.number_format($payment[$a]['price_user'],2,',','').'</td>
											<td class="change_order_total_col">'.number_format($epprice,2,',','').'</td>
											<td class="change_order_total_col">'.number_format($articleprice,2,',','').'</td>
                                            </tr>';
										$total+=$articleprice;
			}
			
            $invoice_details_pdf.='<tr>
											<td style="border-top:1pt solid black;text-align:right;margin-right:10px;font-size: 12pt;" colspan="3">
												Total de la prestation HT
											</td>
											<td style="border-top:1pt solid black;font-size: 12pt;" class="change_order_total_col" >
												'.number_format($total,2,',','').'
	...
	/* Function to generate and download client invoice pdf */
	public function downloadinvoicetest2Action()
    {
        $invoiceid= $_REQUEST['delivery']; 
        $invoicedir='/home/sites/site5/web/FO/invoice/client/'.$_REQUEST['client'].'/';
       ...
       // {
            ...
            //Payment details
            $payment=$pay_obj->getdetailsDeliveryCorrection($invoiceid);
			...
			{
			$epprice=$payment[$a]['price_user'] * ((100-$payment[$a]['contrib_percentage'])/100);
			$articleprice=$payment[$a]['price_user'] + $epprice + $payment[$a]['price_corrector'];
			$invoice_details_pdf.='<tr>
                                            <td>'.$payment[$a]['title'].'</td>
                                            <td class="change_order_total_col">'.number_format($payment[$a]['price_user'],2,',','').'</td>
											<td class="change_order_total_col">'.number_format($payment[$a]['price_corrector'],2,',','').'</td>
											<td class="change_order_total_col">'.number_format($epprice,2,',','').'</td>
											<td class="change_order_total_col">'.number_format($articleprice,2,',','').'</td>
                                            </tr>';
										$total+=$articleprice;
            ...
            $invoice_details_pdf.='<tr>
											<td style="border-top:1pt solid black;text-align:right;margin-right:10px;font-size: 12pt;" colspan="4">
												Total de la prestation HT
											</td>
											<td style="border-top:1pt solid black;font-size: 12pt;" class="change_order_total_col" >
												'.number_format($total,2,',','').'
