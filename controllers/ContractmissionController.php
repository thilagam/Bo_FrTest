...
//				echo "<pre>";print_r($save);exit;
		...
		if($requestparmas['contract_id'])
		{
			...
			if($contract)
			{
				...
				foreach($invoices as $invoice):
					...
					$invoices[$i]['title'] = $this->product_array[$invoice['product']]." ".$this->producttype_array[$invoice['product_type']]." ".$this->getCustomName("EP_LANGUAGES",$invoice['language_source'])." vers ".$this->getCustomName("EP_LANGUAGES",$invoice['language_dest']);
					else
					$invoices[$i]['title'] = $this->product_array[$invoice['product']]." ".$this->producttype_array[$invoice['product_type']]." en ".$this->getCustomName("EP_LANGUAGES",$invoice['language_source']);
								...
								foreach($exploded_file_paths as $row)
								{
									...
									if(file_exists($file_path) && !is_dir($file_path))
									{
										...
										if($fname=="")
											$fname = basename($row);
										$ofilename = pathinfo($file_path);
										$files .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Prod</td><td align="center" width="15%"><a href="/quote/download-document?type=prod_mission&mission_id='.$contractMissionDetails[0]['contractmissionid'].'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="deleteprod" rel="'.$k.'_'.$contractMissionDetails[0]['contractmissionid'].'" id="'.$k.'_'.$contractMissionDetails[0]['contractmissionid'].'"> <i class="icon-adt_trash"></i></span><td></tr>';
									}
									$k++;
								}
							}
				...
				if($params['contractmissionid']):
					...
					if($params['missiontype']=='prod')
					{
						if($qmdetails[0]['product']=='translation')
							$mission_name = $this->product_array[$qmdetails[0]['product']]." ".$this->producttype_array[$qmdetails[0]['product_type']]." ".$this->getCustomName("EP_LANGUAGES",$qmdetails[0]['language_source'])." vers ".$this->getCustomName("EP_LANGUAGES",$qmdetails[0]['language_dest']);
						else
							$mission_name = $this->product_array[$qmdetails[0]['product']]." ".$this->producttype_array[$qmdetails[0]['product_type']]." en ".$this->getCustomName("EP_LANGUAGES",$qmdetails[0]['language_source']);
	...
	function formatMission($row)
	{
		...
		if($row['product']=='translation')
		{
			...
			$array['title'] = $this->product_array[$row['product']]." ".$this->producttype_array[$row['product_type']]." ".$lang_source." vers ".$lang_dest;
		...
		{
			$array['title'] = $this->product_array[$row['product']]." ".$this->producttype_array[$row['product_type']]." en ".$lang_source;
        ...
        if($this->_request->isPost() && $_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
        {
            ...
            foreach($documents_paths as $row)
            {
                ...
                if(file_exists($file_path) && !is_dir($file_path))
                {
                    ...
                    $files .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Staff</td><td align="center" width="15%"><a href="/quote/download-document?type=staff_mission&mission_id='.$identifier.'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="deletestaff" rel="'.$k.'_'.$identifier.'" id="'.$k.'_'.$identifier.'"> <i class="icon-adt_trash"></i></span></td></tr>';
                }
                $k++;
            }
            if($zip && !$zip_req)
    ...
    function deleteDocumentProdAction()
    {
        if($this->_request->isPost() && $_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
        {
            $parmas = $this->_request->getParams();
            ...
            $identifier = $explode_identifier[1];
            $contract_obj=new Ep_Quote_Quotecontract();
            $result = $contract_obj->getContractMission('','prod','',$identifier);//change
            $documents_paths = explode("|",$result[0]['documents_path']);
            $documents_names = explode("|",$result[0]['documents_name']);
            ...
            $data['documents_path']	= implode("|",$documents_paths);
            $data['documents_name']	= implode("|",$documents_names);
            $contract_obj->updateProdMission($data,$identifier);//change
            $documents_paths = array_filter(array_values($documents_paths));
            $documents_names = array_values($documents_names);
            ...
            foreach($documents_paths as $row)
            {
                ...
                if(file_exists($file_path) && !is_dir($file_path))
                {
                    ...
                    if($fname=="")
                        $fname = basename($row);
                    $ofilename = pathinfo($file_path);
                    //$files .= '<div class="topset2"><a href="/quote/download-document?type=tech_mission&mission_id='.$identifier.'&index='.$k.'">'.$fname.'</a><span class="deleteprod" rel="'.$k.'_'.$identifier.'"> <i class="splashy-error_x"></i></span></div>';
                    $files .= '<tr><td width="30%">'.$fname.'</td><td width="35%">'.substr($ofilename['filename'],0,-3).".".$ofilename['extension'].'</td><td width="20%">'.formatSizeUnits(filesize($file_path)).'</td><td>Prod</td><td align="center" width="15%"><a href="/quote/download-document?type=staff_mission&mission_id='.$identifier.'&index='.$k.'"><i style="margin-right:5px" class="splashy-download"></i></a><span class="deleteprod" rel="'.$k.'_'.$identifier.'" id="'.$k.'_'.$identifier.'"> <i class="icon-adt_trash"></i></span></td></tr>';
                }
                $k++;
            }
            if($zip && !$zip_req)
	...
	{
		ini_set('max_execution_time', 300);
		$document_name = "Quote-Quotemissions-import_new.xlsx";
		$document_path= $_SERVER['DOCUMENT_ROOT'].'/BO/oldmissionsupload/'.$document_name;
		...
		if(count($data[0][0])>1)
		{
			for($i=1;$i<count($quote_details);$i++)
			{
			   ...
			   $quotes['client_id'] = $quote_details[$i][2];
			   $quotes['title'] = $quote_details[$i][2];
			   $quotes['title'] = $quote_details[$i][2];
			}
		}
