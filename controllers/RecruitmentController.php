...
class RecruitmentController extends Ep_Controller_Action
{
			...
			{	
				...
				foreach($misssionQuoteDetails as $pkey=>$deliveryMission)
				{
					...
					if(count($this->recruitment_creation->prod_step1)==0)
					{
						...
						$title=$misssionQuoteDetails[$pkey]['product_name']." en ".$language_text." pour un site Internet".$prefix.$category; 					
						...
						$this->recruitment_creation->prod_step1['price_min']=$deliveryMission['min_cost'];
						$this->recruitment_creation->prod_step1['price_max']=$deliveryMission['writing'];
						//for validation
						$this->recruitment_creation->prod_step1['price_min_valid']=$deliveryMission['min_cost'];
						$this->recruitment_creation->prod_step1['price_max_valid']=$deliveryMission['writing'];
						...
						$words_peruser_perdelivery=round($deliveryMission['mission_end_days']/7)*$words_perweek_peruser;
						...
						$this->recruitment_creation->test_article['correction_pricemax']=$deliveryMission['proofreading'];
						//for validation
						$this->recruitment_creation->test_article['correction_pricemin_valid']=0;
						$this->recruitment_creation->test_article['correction_pricemax_valid']=$deliveryMission['proofreading'];						
					...
					$this->recruitment_creation->prod_step1['currency']=$deliveryMission['currency'];
	...
	public function saveProd1Action()
	{
		if($this->_request->isPost())
		{
			...
			if($mission_id)
			{
				$product=$this->recruitment_creation->prod_step1['product']=='redaction' ? 'r&eacute;dacteur' : 'translator';
	...
	{		
		...
		$aowithlink='<a href="http://ep-test.edit-place.com/contrib/aosearch">'.stripslashes($this->recruitment_creation->prod_step1['title']).'</a>';
		$sub=$email[0]['Object'];
	...
	function followUpAction()
	{
		...
		if($misssionQuoteDetails)
		{
			...
			foreach($misssionQuoteDetails as $pkey=>$deliveryMission)
			{
					...
					$misssionQuoteDetails[$pkey]['launch'] = strftime("%d %b %Y",$deliveryMission['recruitment_launch']);
					if($deliveryMission['proofread_end'])
						$misssionQuoteDetails[$pkey]['enddate'] = strftime("%d %b %Y",strtotime($deliveryMission['proofread_end']));
					else
					{
						$days = ceil($misssionQuoteDetails[$pkey]['total_time']/(60*24));
						$misssionQuoteDetails[$pkey]['enddate'] = strftime("%d %b %Y",strtotime("+$days days",$deliveryMission['recruitment_launch']));
					}
