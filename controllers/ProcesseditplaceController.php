...
class ProcesseditplaceController extends Ep_Controller_Action
{
	...
	private $data_path;//Added by arun
	...
	public function init()
	{
		...
		/*added w.r.t FO pages   change the same in page.php,module.php,pattern.php too*/	
		
		if($_GET['segment']==3)
			$this->data_path=DATA_PATH_FO;
		else
		...
		$this->my_obj = new Ep_Db_ArrayDb2($this->data_path."main_meta_data.xml", "root");
		$this->adminLogin	= Zend_Registry::get ( 'adminLogin' );
		$this->_view->loginName = $this->adminLogin->loginName;
		...
		$this->versionFile = $this->data_path."version.xml";
		 ///////////////////////////////////////////////////////////////////////
		...
		//////////////////////////////////////////////////////////////////////
		$this->patternFile = $this->data_path.'pattern.xml';
		$this->moduleFile = $this->data_path.'module.xml';
	}
	...
	public function pageAction()
	{
		...
		$this->_view->lastsection = count($sectionList);   //print_r($this->_request->getParam); exit;
		...
		if($do == "add")
		{
			...
			{    // print_r($this->_request->getParam()); echo "in edit"; exit;
				if($update_check == 1)
				echo $p->update();
				else
				{
    ...
    public function changepagedescAction()
    {
        $p = new Ep_Controller_Page($this->pageFile);
        $page = trim($this->_request->getParam("page"));
         $page = str_replace(" ", "_", $page);
         $p->getNodeMap($page);
         $p->setName($page);
         $p->setDescription(stripslashes(utf8_encode($this->_request->getParam("desc"))));
         $p->update();
    }
	/**
