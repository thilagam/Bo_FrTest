...
class OngoingController extends Ep_Controller_Action
{
       ...
       // print_r($ongoingAoList); exit;
      	if($ongoingAoList)
		{
            foreach($ongoingAoList as $key=>$value)
            {
                ...
                $progresspercentage = round($ongoingAoList[$key]['avgprogpercentage']);
		...
		/*if($_GET['manager_id']!="")
			$this->_view->manager_id=$_GET['manager_id'];
		elseif($_GET['manager_id']!=0 && $_GET['manager_id']!="")
			$this->_view->manager_id=$this->adminLogin->userId;*/
        ...
        { 
        	...
        	if($ao_id && $client_id)
        	{
				...
				/*if($this->adminLogin->groupId==1)
                    $updateArray['ao_visibility']=$editParams['ao_visibility'] ? 'hide' : 'show';*/
				...
				if($editParams['correction'] == 'external')
				{
					...
					$updateArticleArray['correction_type']='extern';
    	...
    	if($this->_request-> isPost())            
        {
        	...
        	{      
				...
				if($updateArray['correction']=='yes')
	        	{
	        		...
	        		//$pa_array=array("current_stage"=>'corrector', "correction_participationexpires"=>$expires);
	        		$pa_array=array("current_stage"=>'corrector');
	        		$query = "article_id= '".$article_id."' AND status = 'under_study' AND current_stage IN ('stage1','stage2')";       		
	        	...
	        	elseif($updateArray['correction']=='no')
	        	{
                   ...
                   /* if($editParams['part_id'])
                    {
                    	//updating Participation table for user prices ///
                    	$pricearray=array("price_user"=>str_replace(",",".",$editParams['price_writer']));
                    	$queryprice = "article_id= '".$article_id."' AND id = '".$editParams['part_id']."'";
                    	$participate_obj->updateParticipation($pricearray,$queryprice);
                    }	*/
    ...
    public function downloadSpecfileAction()
    {
        ...
        if(isset($spec_path))
        {
            ...
            $old_specpath = "/home/sites/site5/web/FO/client_spec".$details[0]['filepath'];
    ...
    public function downloadArticleAction()
    {
        ...
        if($type=='process' && $process_id)
        {
            ...
            $old_file_path = "/home/sites/site5/web/FO/articles/".$article_path;
    ...
    public function getPicPath($identifer,$action='home')
    {
        ...
        {
            $pic_path="http://ep-test.edit-place.com/".$profiledir.$pic;
        }
        else
        {
            if($action=='home' OR $action=='bo_user')
              $pic_path="http://ep-test.edit-place.com/FO/images/editor-noimage_60x60.png";
            else
              $pic_path="http://ep-test.edit-place.com/FO/images/editor-noimage.png";
        }
        return $pic_path;
    }
    ...
    public function getClientPicPath($identifer,$action='home')
    {
        ...
        {
           if($action=='home')
            $pic_path="http://ep-test.edit-place.com/FO/images/customer-no-logo.png";
           else
             $pic_path="http://ep-test.edit-place.com/FO/images/customer-no-logo90.png";
        }
        return $pic_path;
    }

    // Ongoing Super Client list

    public function superClientListAction()
    {
		$aoObject=new EP_Ongoing_Delivery();
		...
		$superClientongoingAoList=$ref_obj->OngoingDeliveryRefDetails($searchParams);
		
		if($superClientongoingAoList)	
		{
			//to get bo contacts
			for($s=0;$s<count($superClientongoingAoList);$s++)
			{
				$superClientongoingAoList[$s]['contacts']=$sccontact_obj->getBousers($superClientongoingAoList[$s]['id']);
			}
			$this->_view->ongoingAO=$superClientongoingAoList;
		}
		
		$this->_view->usertype = $this->adminLogin->type; 
    	$this->_view->render("ongoing_super_client_list");		
    }
    //ongoing  details of a Delivery
    public function superClientAoDetailsAction()
    {
    	$aoParams=$this->_request->getParams();
    	$aoObject=new EP_Ongoing_Delivery();
    	$articleObject=new EP_Ongoing_Article();
		$comments_obj=new Ep_Delivery_Adcomments();
		$ref_obj=new Ep_User_DeliveryReference();
    	$ref_id=$aoParams['ref_id'];
    	$aoParams['sorttype']='all';
	
    	if($ref_id)
    	{
			$superClientongoingAoDetails=$ref_obj->OngoingDeliveryRefDetailseach($ref_id);	
			
			if($superClientongoingAoDetails)
			{
				if($superClientongoingAoDetails[0]['access_client'])
					$clients_count=count(explode(",",$superClientongoingAoDetails[0]['access_client']));
			}	
			...
			if($superClientongoingAoDetails)	
			{
				//getting All article details of AO
				$aoParams['missiontest']=$aoDetails[0]['missiontest'];
				$articleDetails=$articleObject->OngoingBOUserArticleDetails($aoParams);
				...
				{	
					...
					$superClientArticleDetails=array(); 
					...
					for($i=0;$i<$clients_count;$i++)
					{
						...
						if(count($articleDetails)>0)
						{
							foreach($articleDetails as $article)
							{
								...
								$superClientArticleDetails[$article['user_id']]['client']=$article['client'];						
						
								//$superClientArticleDetails[$article['user_id']][$article['delivery_id']]['delivery_title']=$article['delivery_title'];
								$superClientArticleDetails[$article['user_id']][$article['delivery_id']][$article['id']]=$article;
				...
				//echo "<pre>";print_r($superClientArticleDetails);exit;

				//All super clients/clients list
				
				$reference_array=$ref_obj->ListReference();
				if($reference_array)
				{
					foreach($reference_array as $ref)
					{
						$ref_array_list[$ref['id']]=$ref['title'];
					}

				}
				//echo "<pre>";print_r($clients_array_list);

				
				$this->_view->ref_array_list=$ref_array_list;
				$this->_view->aoDetails=$superClientongoingAoDetails;
				$this->_view->superClientArticleDetails=$superClientArticleDetails;
				$this->_view->render("ongoing_super_client_ao_details");	
			}
			else
    			$this->_redirect("/ongoing/super-client-list?submenuId=ML2-SL26");
    ...
    public function downloadContractAction()
    {
    	...
    	if($user_id OR $article_id)
    	{
            ...
            //echo "<pre>";print_r($participation_watch_list);exit;
    ...
    {   
		...
		{	
			...
			{
				critsendMail($this->adminLogin->loginEmail, $UserDetails[0]['email'], $object, $content);
				return true;
			}
			else
			{
