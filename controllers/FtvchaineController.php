...
class FtvchaineController extends Ep_Controller_Action
{
    ...
    // editing the request created by client //
    public function editrequestAction()
    {
        ...
        if($request_params['request_id'] != '' && $request_params['edit_demand'] == 'yes')
        {
            ...
            $this->_redirect("/ftvchaine/ftvch-requests?submenuId=ML11-SL6");
        }
    ...
    public function duplicaterequestAction()
    {
        ...
        $this->_view->ftvchaineeditrequest = 'yes';
        $this->render("ftv_ftvaddcomment");
    }
    public function ftvchRequestsAction()
    {
    ...
    public function savecommentsAction()
    {
        ...
        if($_SERVER['HTTP_X_REQUESTED_WITH']=='XMLHttpRequest')
        {
            ...
            if($request_id != '' && $comments != '')
            {
                ...
                $automail->sendFtvChaineContactsPersonalEmail($requestdetails[0]['request_by'],115,$parameters); // to client contact
                ...
                foreach($othercontacts as $values)
                {
                    ...
                    $automail->sendFtvChaineContactsPersonalEmail($values,115,$parameters); // to client contact
                }
            }
            /*$commentDetails=$ftvcomments_obj->getCommentsByRequests($request_id);
            $commentsData='';
    ...
    // ftv contact creations ///
    public function ftvchContactCreationAction()
    {
        $ftv_obj=new Ep_Ftv_FtvContacts();
        $user_params=$this->_request->getParams();
        ...
        if(isset($contactid) && $contactid != '')
        {
            ...
            if($user_params["edit"] == 'yes')
            {
                ...
                if($emailexit == 'yes')
                {
                    ...
                    $this->_redirect("/ftvchiane/ftvch-contacts?submenuId=ML11-SL4");
                }
                else
                {
                    $data = array( "email_id"=>$user_params["email"], "password"=>$user_params["password"], "first_name"=>$user_params["first_name"], "last_name"=>$user_params["last_name"]);
                    $query = "identifier= '".$contactid."'";
                    $ftv_obj->updateFtvContacts($data,$query);
                    $this->_helper->FlashMessenger('Contact Edited Successfully.');
                    $this->_redirect("/ftvchaine/ftvch-contacts?submenuId=ML11-SL4");
                }
            }
        }
        if($this->_request->isPost())
