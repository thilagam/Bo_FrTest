...
class CorrectionController extends Ep_Controller_Action
{
    ...
    public function correctorArticleProfilesAction() {
        ...
        if ($aoId != NULL) {
            if(isset($partParams['crtstatus']))
            {
                $condition['crtstatus'] = $partParams['crtstatus'];
            }
            $condition['aoId'] = $aoId;
    ...
    public function correctorGroupProfilesAction() {
        ...
        if ($partParams['artId'] != NULL) {
           ...
           /* $partStatus=$participate_obj->articleProfiles($artId);
            ...
            { $this->_redirect("http://admin-test.edit-place.com/processao/article-profiles?submenuId=ML2-SL2&aoId=".$delDetails[0]['id']); }
    ...
    public function getcommentpopupAction()
    {
		...
		$parameters['aoname_link']="http://ep-test.edit-place.com/contrib/mission-corrector-deliver?article_id=".$art_id;
		...
		if($delivery_details[0]['contract_mission_id'])
		{
			$max=max($delivery_details[0]['cjt'], $delivery_details[0]['cst']);
    ...
    public function republishcorrectorpopupAction()
    {
        ...
        if($republishParams['save'] == 'save')    ////saving the details to DB ///
        {
        ...
        if($artdeldetails[0]['corrector_privatelist'] == '')      ///this ao is private
        {
            ...
            $aoprofiles=$delivery_obj->getViewToOfAO($pubprofiles); /// based on the profile type //
    ...
    public function extendCrtPartExpireTimeAction()
    {
        $prevurl = getenv("HTTP_REFERER");
        $articleParams=$this->_request->getParams();
        $this->_view->artId = $articleParams['artId'];
        $mode = $articleParams['mode'];
        $participation_obj=new EP_Participation_Participation();
        $delivery_obj = new Ep_Delivery_Delivery();
        $article_obj = new Ep_Delivery_Article();
        $artdeldetails = $delivery_obj->getArtDeliveryDetails($articleParams['artId']);
        $this->_view->crtpartexpdate = date("d-m-Y", $artdeldetails[0]['correction_participationexpires']);
        $this->_view->crtpartexptime = date("g:i A", $artdeldetails[0]['correction_participationexpires']);
        if($articleParams['crtextensionsubmit'] == 'crtextsubmit')   ///when time changes in mail content in publish ao popup//
        {
            $extdate = date('m/d/Y', strtotime($articleParams['extend_crtpartdate']));
            $exttime = date('H:i', strtotime($articleParams['ext_crtparttime']));
            $extenddatetime =strtotime($extdate." ".$exttime);
            $data = array("correction_participationexpires"=>$extenddatetime);
            $query = "id= '".$articleParams['artId']."'";
            $article_obj->updateArticle($data,$query);
            $this->_redirect($prevurl);
        }
        $this->_view->extendcrtparttime   = 'yes';
    ...
    public  function plagarismAction()
    {
        ...
        $cron = $cron_obj->getCronLock('plagarism'); // for locking the cron //
        ...
        else
        {
			...
			if($partArtProcDetails != 'NO')
			{
				for($i=0; $i<$partArtProcDetailsCount; $i++)
				{
					...
					if(file_exists($srcFile))
					{
						...
						if($response == 'File Size is more Than 800kb')
						{
                            ...
                            $maildetials[$i]['plag_stuck'] = $partArtProcDetails[$i]['plag_stuck'];
                            $plagstuck[$i] = $partArtProcDetails[$i]['plag_stuck'];
                            $sendmail = 'yes';
                            /////stuck up///
                            $artProcess_obj = new EP_Delivery_ArticleProcess();
                ...
                if($sendmail == 'yes' && in_array('no', $plagstuck))
				...
				if(count($resultarr)>0)
				{
					...
					$filename = "/home/sites/site6/web/BO/documents/plagresult.xls";
					$this->WriteXLS($resultarr, $filename);
					$filename = "plagresult.xls";
					$path="/home/sites/site6/web/BO/documents/";
                    ...
                    $msgdetails = "<table border='1'><tr><th>ArticleName</th><th>AoTitle</th><th>Contributor</th><th>Plagiarism Result</th><th>Current Stage</th><th>Article Type</th></tr>";
                    for($i= 0; $i < count($resultarr); $i++){
                        $msgdetails.= "<tr><td>".$resultarr[$i]['artName']."</td><td>".$resultarr[$i]['aoName']."</td>";
                        $msgdetails.= "<td>".$resultarr[$i]['email']."</td><td>".$resultarr[$i]['plagpercent']."</td>";
                        $msgdetails.= "<td>".$resultarr[$i]['currentstage']."</td><td>".$resultarr[$i]['ext']."</td></tr>";
                    }
                    $msgdetails.= "</table>";
                    ///sending personal mails to chandu and manohar///
                    $Message="<p>Hi,<br><br>
									Plagiarism Results :<br><br>
									".$msgdetails."<br><br>
									 Cordialement,<br><br>
                    ...
                    if($resultarr[0]['aoName'] != '')  /// when results are there//
                        $mail->send();
                }
			}
			else
    ...
    public function plagstuckupmail($maildetials, $articlepath)
    {
        ...
        $Message="<p>Hi,<br><br>
                            ...
                            link to palgiarism stage : <a href=\"http://admin-test.edit-place.com/proofread/stage-articles?submenuId=ML3-SL11&aoId=".$maildetials['aoid']."\">Stage Plagiarism</a><br>
    ...
    public function plagstuckupcronmail($maildetials)
    {
      $msgdetails = "<table border='1'><tr><th>AO Title</th><th>Article Title</th><th>Plagiarism Stage</th><th>Stuck files</th></tr>";
        for($i= 0; $i < count($maildetials); $i++){
            if($maildetials[$i]['aoname'] != '' && $maildetials[$i]['plag_stuck'] == 'no')
            {
                $msgdetails.= "<tr><td>".($maildetials[$i]['aoname'])."</td><td>".($maildetials[$i]['artname'])."</td>";
                $msgdetails.= "<td><a href=\"http://admin-test.edit-place.com/proofread/stage-articles?submenuId=ML3-SL11&aoId=".$maildetials[$i]['aoid']."\">Stage Plagiarism</a></td>";
                $msgdetails.= "<td><a href=\"http://admin-test.edit-place.com/proofread/plag-stuck-arts?submenuId=ML3-SL13\">List of stuck up files</a></td></tr>";
            }
        }
        $msgdetails.= "</table>";
        ///sending personal mails to chandu and manohar///
    ...
    public function uploadAndProcess($srcFile,$u_filename,$filename,$cron, $partArtProcDetails)
    {
        ...
        $cmdfilename = "/home/sites/site6/web/BO/plagarism/cmdfile.txt";
        ...
        //echo  "<br>".$localFile=APP_PATH_ROOT."plagarism/".$xmlfilefolder."/";exit;
    ...
    /// rading the xml after plagiarism process and getting the percentages of plagiarism ///
    ...
    // plagiarism detials extracted from xml ////
    ...
    public function moderationAction()
    {
        ...
        ///////////////////////////////////////////////////////////////////////////
        if(isset($partParams["moderate_dirvalidate"])) ///when direct validate id clicked
        {
            //////////////////////////////////////////////////////////////////////////
            ...
            $data = array("moderate_epdecision"=>"directvalid");////////updating
            ...
            $data = array("current_stage"=>"stage2", "status"=>"under_study");////////updating
            $query = "id= '".$partId."'";
            $participate_obj->updateParticipation($data,$query);
            ////udate status corrector participation table for stage///////
            $data = array("current_stage"=>"stage2", "status"=>"under_study");////////updating
            $query = "participate_id= '".$partId."'";
            $correctorparticipate_obj->updateCrtParticipation($data,$query);

            //////sending mail to corrector///////////////
            $correctorId = $partParams['correctorId'];
            $email=$automail->getAutoEmail($partParams["dirvalidmailid"]);
            $Object = $email[0]['Object'];
            $automail->sendMailEpMailBox($correctorId,$Object,$partParams["Moderator_dirvalcomment"]);  ///sending mail to EP account
          ...
          //  $this->articleHistory(44,$actparams);
            /////////////end of article history////////////////
        ...
        if(isset($partParams["moderate_disapprove"])) ///when refuse link id clicked
        {
            ...
            $query = "participate_id= '".$partId."' AND status = 'bid' AND cycle = 0 ";
            $correctorparticipate_obj->updateCrtParticipation($data,$query);
        ...
        {   //echo "hi"; print_r($partParams); exit;
            ...
            if($partParams["status"] == "disapproved_temp")
            {    //echo "hi 1"; exit;
                $data = array("current_stage"=>"contributor", "status"=>"disapproved", "article_submit_expires"=> $expires,
                ...
                $parameters['article_link']="http://ep-test.edit-place.com/contrib/refused";
    ...
    /////////////////////when corrector disapproves from FO it will come to bulk moderation page for BO approval///////////////////
    public function moderationbulkAction()
    {
        $prevurl = getenv("HTTP_REFERER");
        $userplus_obj=new Ep_User_UserPlus();
        $automail=new Ep_Message_AutoEmails();
        ...
        $delivery_obj = new Ep_Delivery_Delivery();
        $participate_obj = new EP_Participation_Participation();
        $artlist = explode(",",$partParams['artlist']);
        ///////////////////////////////////////////////////////////////////////////
        if(isset($partParams["moderate_dirvalidate"])) ///when direct validate id clicked
        {
            for($i=0; $i<count($artlist); $i++)
            {
                //////////////////////////////////////////////////////////////////////////
                $partsOfArt= $participate_obj->getAllParticipationsCorrectorDisapprove($artlist[$i]);
                $partId =  $partsOfArt[0]['id'];
                //Added By arun
                $prt_details=$participate_obj->getParticipateDetails($partId);
                if($prt_details[0]['user_id'])
                ...
                $crtpartsOfArt= $correctorparticipate_obj->getParticipationsCorrectorToDisapprove($artlist[$i]);
                $crtpartId =  $crtpartsOfArt[0]['id'];
                $partdetials = $correctorparticipate_obj->getCrtParticipationsStatus($crtpartId);
                $partStatus = $partdetials[0]['status'];
                $partUser =  $partdetials[0]['corrector_id'];
                ...
                $refusedcountupdated++;
                $partuserdetials = $userplus_obj->getUsersDetailsOnId($partUser);
                $deliveryDetails = $delivery_obj->getArtDeliveryDetails($artlist[$i]);
                ...
                $parameters['correcteddate']=date("d/m/Y H:i",strtotime($contributor_participation[0]['updated_at']));
                $parameters['article_title']=$deliveryDetails[0]['articleName'];
                $parameters['articlename_link']="/contrib/mission-deliver?article_id=".$artlist[$i];
                $parameters['article_link']="http://ep-test.edit-place.com/contrib/refused";
                $correctorId = $partParams['correctorId'];
                $parameters['moderatorcomments']=$partParams["Moderator_dirvalcomment"];
                $automail->messageToEPMail($correctorId,$partParams["mail_id"],$parameters);  //    sending mail to corrector
                /*$email=$automail->getAutoEmail($partParams["dirvalidmailid"]);
                $Object = $email[0]['Object'];
                $automail->sendMailEpMailBox($correctorId,$Object,$partParams["Moderator_dirvalcomment"]);  ///sending mail to EP account*/
        ...
        {   
            for($i=0; $i<count($artlist); $i++)
            {
               ...
               // $parameters['comments']=$partParams['Moderator_comment'];
                ...
                $parameters['articlename_link']="/contrib/mission-deliver?article_id=".$artlist[$i];
                $parameters['article_link']="http://ep-test.edit-place.com/contrib/refused";
                $parameters['moderatorcomments']=$partParams["Moderator_comment"];
                $correctorId = $partParams['correctorId'];
                $automail->messageToEPMail($correctorId,$partParams["mail_id"],$parameters);//
        ...
        if(isset($partParams["moderate_approve"]))///when accept link id clicked
        {
            for($i=0; $i<count($artlist); $i++)
            {
                ...
                if($partParams["status"] == "disapproved_temp")
                {
                    ...
                    $automail->messageToEPMail($contribId,57,$parameters);//    sending mail to contributor
                    $correctorId = $partParams['correctorId'];
                    //  print_r($partParams);  exit;
                    //$email=$automail->getAutoEmail($partParams["acceptmailid"]);
                    //$Object = $email[0]['Object'];
                    $parameters['moderatorcomments']=$partParams["Moderator_crtwindow"];
                    $automail->messageToEPMail($correctorId,$partParams["mail_id"],$parameters);  //    sending mail to corrector
                    //$automail->sendMailEpMailBox($correctorId,$Object,$partParams["Moderator_crtwindow"]);  ///sending mail to EP account
                    /////////////article history////////////////
                    $actparams['contributorId'] = $correctorId;  ///this corrector who did refused definite from FC///
                ...
                else
                {
                    if($partParams["sendtofo"] == 'yes')
                    {
                        $cycleCount = $participate_obj->getParticipationCycles($artlist[$i]);
                        $cycleCount = $cycleCount[0]['cycle']+1;
                        /////udate status participation table with article id///////
                        $data = array("cycle"=>$cycleCount, "moderate_closed"=>"yes");////////updating
                        $query = "article_id= '".$artlist[$i]."' and cycle=0";
                        $participate_obj->updateParticipation($data,$query);
                        ////update the artlcle table with partcipation time/////////
                        $this->WriterParticipationExpire($artlist[$i]);
                        $data = array("current_stage"=>"corrector", "status"=>"closed",  "moderate_closed"=>"yes",
                            "marks"=>$partParams['latestmarks']);////////updating
                        $query = "id= '".$partId."'";
                        $participate_obj->updateParticipation($data,$query);
                        if($partParams["anouncebyemail"] == 'yes')
                            $this->sendMailToContribs($artlist[$i]);
                    }
                    elseif($partParams["sendtofo"] == 'no')
                    {
                        ////updating the article tabel article submit expire wiht zero///////
    ...
    public function moderationCorrectionAction()
    {
        ...
        if($artId!=NULL)
        {
			...
			//$this->_view->contribmarks=$participate_obj->getContributorMarks($versions_details[0]['participate_user']);
        }
    ...
    public function getmoderatemailcontentAction()
    {
        ...
        elseif($partParams["actionmode"] == "refuse")
        {
            //////sending mail to contributor///////////////
            if($resubmission <= '60')
                $parameters['resubmit_hours']=$resubmission." minutes";
    ...
    // check if participated  corrector is last left one ////
    ...
    public function sendPlagEpMailBox($partArtProcDetails, $plagged, $plag_percent)
    {
        ...
        if($plagged)
        {
            $text_mail="<p>Cher chef de projet,<br><br>
 
L'article <b>" . $user[0]['title'] . "</b> se trouve actuellement en phase plagiat sur le BO Edit-Place car il a &#233;t&#233; d&#233;tect&#233; que <b>" . $plag_percent . " %</b> du contenu &#233;tait plagi&#233;.<br> 

<a href=\"http://admin-ep-test.edit-place.com/proofread/stage0-correction?submenuId=ML3-SL11&articleId=".$partArtProcDetails['article_id']."&participateId=".$partArtProcDetails['partId']."\">Cliquez ici</a> pour vous rendre en phase plagiat et analyser l'article.<br><br>


Cordialement,<br><br>


Toute l'&#233;quipe d'Edit-Place"
            ;
            $subject= "BO Edit-Place - Article plagié à analyser";
        }
        else 
        {
            $text_mail="<p>Cher chef de projet,<br><br>
 
L'article <b>" . $user[0]['title'] . "</b> se trouve actuellement en phase plagiat sur le BO Edit-Place car le type de fichier soumis ne peut &#234;tre trait&#233; automatiquement.<br> 

<a href=\"http://admin-ep-test.edit-place.com/proofread/stage0-correction?submenuId=ML3-SL11&articleId=".$partArtProcDetails['article_id']."&participateId=".$partArtProcDetails['partId']."\">Cliquez ici</a> pour vous rendre en phase plagiat et analyser l'article manuellement.<br><br>
...
Toute l'&#233;quipe d'Edit-Place"
            ;
            $subject= "BO Edit-Place - Article bloqué en phase plagiat";
        }
        ...
        $mail->setBodyHtml($text_mail)
            ...
            ->addCc('anoopchandanathope@gmail.com')
    ...
    public function testexceltxtAction() {
        $params = $this->_request->getParams();
        require_once SEO_FILE_CONVERTION;
        $content = new filecontent(APP_PATH_ROOT . 'plagarism/plagcsvtest.xls', APP_PATH_ROOT . 'plagarism/plagcsvtest_xls.txt', 'xls2txt', 2);
        $content = new filecontent(APP_PATH_ROOT . 'plagarism/plagcsvtest.xlsx', APP_PATH_ROOT . 'plagarism/plagcsvtest_xlsx.txt', 'xlsx2txt', 2);
        $status = $content->getStatus();
    }

    public function testxmlpercentageAction() {
        $params = $this->_request->getParams();
        $xml_data=$this->XMLParserPercentage(APP_PATH_ROOT . 'seo_download/plagiarism/' . $params['file']);
        $xmlpercentage=array();
	...
	/// locking the cron until the process completed  ///
	...
	public function downloadSelectedCorrectorxlsAction() 
	{
		...
		$articles=$delivery_obj->missionarticlelist($ao_id);
                 ...
                 {
                     $details=$delivery_obj->missionselectedcorrectors($articles[$p]['id']);
					 
					 $name=$details[0]['first_name'].' '.$details[0]['last_name'];
					
					 if($details[0]['profile_type2']!="")
					 {
					 ...
					 echo '<tr>
								...
								<td valign="top">'.$details[0]['profile_type2'].'</td>
								<td valign="top">'.$details[0]['price_corrector'].'</td>
