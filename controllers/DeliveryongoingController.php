    ...
    public function saveDeliveryAction()
    {
        ...
        { 
        	...
        	if($ao_id && $client_id)
        	{
                ...
                /* *** on 23.03.2016 *** */
                //update article history if correvtion is changed//
                $articles = $deliveyObj->getDeliveryArticleDetails($ao_id);
                //inserting in acticlehistory if correction is changed.
                for($i = 0; $i <count($articles); $i++) {
                    $oldcorrectionVal = ($articles[$i]['correction'] === 'yes') ? 'yes' : 'no';
                    $editcorrectionVal = ($editParams['correction'] === 'external') ? 'yes' : 'no';
                    if ($oldcorrectionVal != $editParams['correction']) {
                        $actionId = 69;
                        $actparams['artId'] = $articles[$i]['id'];
                        $actparams['stage'] = 'ongoing';
                        $actparams['action'] = 'correction_update';
                        $actparams['old_correction'] = $oldcorrectionVal;
                        $actparams['new_correction'] = $editcorrectionVal;
                        $this->articleHistory($actionId, $actparams);
                        $query=" id='".$articles[$i]['id']."'";
                        if($editParams['correction'] === 'external')  $updateArticleArray['correction_type']= 'extern'; else $updateArticleArray['correction_type'] = 'internal';
                        $updateArticleArray['correction']=$editcorrectionVal;
                        $articleObj->updateArticle($updateArticleArray,$query);//also update correction type in article lever
                    }
                }
//                echo "<pre>";print_r($articles);exit;
    ...
    public function saveArticleAction()
    {
        ...
        {
        	$editParams=$this->_request->getParams();
//            echo "<pre>";print_r($editParams);//exit;
        	...
        	{      
                ...
                /* *** edited on 23.03.2016 ** */
				//Add article history when price range is updated
				if((!empty($editParams['price_min']) && $ArticleDetails[0]['price_min']!=$updateArray['price_min']) || (!empty($editParams['price_max']) &&  $ArticleDetails[0]['price_max']!=$updateArray['price_max']))
				{
					$actionId=64;
                ...
                /* *** added on 23.03.2016 *** */
                //inserting in acticlehistory if correction is changed.
                $oldcorrectionVal = ($ArticleDetails[0]['correction'] === 'yes') ? 'on' : '';
                $editcorrectionVal = ($editParams['correction'] === 'on') ? 'yes' : 'no';
                if($oldcorrectionVal!=$editParams['correction'])
                {
                    $actionId=69;
                    $actparams['artId']=$article_id;
                    $actparams['stage']='ongoing';
                    $actparams['action']='correction_update';
                    $actparams['old_correction']=$ArticleDetails[0]['correction'];
                    $actparams['new_correction']=$editcorrectionVal;
                    $this->articleHistory($actionId, $actparams);
                }
                if($updateArray['correction']=='yes')
	        	{
					...
					if((!empty($editParams['correction_pricemin']) && $ArticleDetails[0]['correction_pricemin']!=$updateArray['correction_pricemin']) || (!empty($editParams['correction_pricemax']) &&$ArticleDetails[0]['correction_pricemax']!=$updateArray['correction_pricemax']))
					{
						$actionId=65;
						$actparams['artId']=$article_id;
						$actparams['stage']='ongoing';
