 ...
 class QuotesCronController extends Zend_Controller_Action
 {
	...
	public function init()
	{
       ...
       $this->closedreason = array(
		...
		$this->_view->fo_path=$this->fo_path='http://ep-test.edit-place.com/FO/';
	}
	...
	function salesDelayAction()
	{
		...
		foreach($quotes as $quote):
			//$users = (array) $quotecontract_obj->getUsers("'salesmanager' OR type='salesuser'",'',true,$quote['quote_by']);
			$user = $quotecontract_obj->getUsers("'salesmanager' OR type='salesuser'",'',true,$quote['quote_by']);
			
			$quote_name = "<strong>".$quote['title']."</strong>";
			$click_here = "<a href='".$this->url."/quote/sales-final-validation?quote_id=".$quote['identifier']."&submenuId=ML13-SL2'>cliquer ici</a>";
			//foreach($users as $user):
				$bo_user = $user[0]['first_name']." ".$user[0]['last_name'];//echo $bo_user;
				eval("\$msg= \"$orgmsg\";");
				//$mail_obj->sendEmail($email_config,$this->mail_from,$user['email'],$subject,$msg);
	...
	*/
	...
	/**To get tempos weekly old function*/
	function getTemposWeeklyOldAction()
	{
		$cron_obj = new Ep_Quote_Cron();
		$contractmissions = $cron_obj->getArticleCount();
		//echo '<pre>'; print_r($contractmissions);//exit;
	...
	function relancerEmailAction(){
								...
								$quoteDetails['signature']=round($signature/$quotetotal);
									...
									$email_head_sale=array("120206112651459"=>"cleguille@edit-place.com",
									                    "110920152530186"=>"mfouris@edit-place.com",
									                     "111113163826982"=>"jwolff@edit-place.com"); // need to add thaibault
		...
		function salesFinalanswerTimeAction(){
			...
			$this->_redirect("/BO/quotes_weekly_report/sales-final-report-on-$lastsixmonth.xlsx");
		...
		if($result)
		{
			...
			foreach($result as $row)
			{
				...
				$htmltable .= "<tr>
								...
								<td >http://".$_SERVER['HTTP_HOST']."/contractmission/contract-edit?submenuId=ML13-SL3&contract_id=".$row['quotecontractid']."&action=view</td>
								<td >".$mtitle."</td>
								<td >".$language."</td>
								<td >".$tempo."</td>								
			...
			$filename = "wp-FRTEST-extract-".$previous_month."-".$year.".xlsx";
			...
			$this->sendMail("arunravuri@edit-place.com", "Sales Report for the month of $previous_month $year at FR TEST", "PFA", $path, $filename,"kavithashree.r@gmail.com");
			unlink($cfilename);
		}
	}
	/*only sales quote missions extract*/
	...
	/*Import Quote mission into prod mission table*/
	function importQuoteMissionAction()
	{
		...
		$userId='110823103540627';
            ...
            $document_name = "Onlysales-QuoteMissions-FR.xlsx";
	...
	/*extract of all Contract missions with History BO status*/
	function extractHistoryboAction()
	{
		$quotecron_obj = new Ep_Quote_Cron();
		$missionDetails=$quotecron_obj->getHistoryBOQuoteMissions();

		if($missionDetails)
		{
			$detailsHtml='<table border=1>
							<tr>	
								<th>Contract Mission Id</th>
								<th>Quote Mission Id</th>
								<th>Client</th>
								<th>Mission</th>
								<th>Type</th>
								<th>Language</th>
			...
			{
				$product=$this->product_array[$mission['product']];
				$productType=$this->producttype_array[$mission['product_type']];

				$language_source=$this->getLanguageName($mission['language_source']);
				$language_dest=$this->getLanguageName($mission['language_dest']);
				if($mission['product']=='translation')
					$anguage=$language_source.' &gt;'.$language_dest;
				else
					$anguage=$language_source;

				$tempo=$mission['oneshot']=='yes'?'Oneshot' : 'Recurring';

				$detailsHtml.='<tr>	
								<td>'.$mission['contractmissionid'].'</td>
								<td>'.$mission['type_id'].'</td>
								...
								<td>'.$mission['WritingCost'].'</td>
								<td>'.$mission['ProofreadingCost'].'</td>
								<td>'.$mission['volume'].'</td>
								<td>'.$mission['Turnover'].'</td>
								<td>'.$mission['Currency'].'</td>
								<td>'.$tempo.'</td>
								<td>'.$mission['history_bo'].'</td>
							</tr>	
						';
