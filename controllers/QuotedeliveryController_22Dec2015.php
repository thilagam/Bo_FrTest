...
{		
    	...
    	if($delivery_id)
    	{
    		...
    		if($deliveryDetails)
    		{
    			...
    			foreach($deliveryDetails as $delivery)
    			{
					...
					{
						$this->delivery_creation->prod_step1['category'] = $delivery['ebooker_cat_id'];
			...
			{	
				...
				foreach($misssionQuoteDetails as $pkey=>$deliveryMission)
				{
					...
					if(count($this->delivery_creation->prod_step1)==0)
					{
						...
						$this->delivery_creation->prod_step1['price_min']=$deliveryMission['min_cost'];
						$this->delivery_creation->prod_step1['price_max']=$deliveryMission['writing'];
						//for validation
						$this->delivery_creation->prod_step1['price_min_valid']=$deliveryMission['min_cost'];
						$this->delivery_creation->prod_step1['price_max_valid']=$deliveryMission['writing'];
						...
						$this->delivery_creation->prod_step1['correction_pricemax']=$deliveryMission['proofreading'];
						//for validation
						$this->delivery_creation->prod_step1['correction_pricemin_valid']=0;
						$this->delivery_creation->prod_step1['correction_pricemax_valid']=$deliveryMission['proofreading'];
					...
					if($this->delivery_creation->prod_step1['theme'])
					{
						...
						$sampletexts = $ebooker_obj->getStencils(array('cat_id'=>$this->delivery_creation->prod_step1['category'],'sampletexts'=>true,'sample_id'=>$this->delivery_creation->prod_step1['sampletext']));
		...
		if($this->delivery_creation->prod_step1['total_article']>0 && $mission_id)
		{
			...
			for($i=1;$i<=$total_article;$i++)
			{
				if(!$this->delivery_creation->prod_step2['articles'][$i]['title'])				
				{
					...
					$article_details['title']="Lot de ".$files_pack." ".$product_type." - ".$this->delivery_creation->prod_step1['client_name']." - ".$i;
	...
	public function saveProd3Action()
	{
		if($this->_request-> isPost())
		{
			...
			$this->delivery_creation->prod_step3['correctormailsubject']=$notificationParams['correctormailsubject'];
			$this->delivery_creation->prod_step3['correctormailcontent']=$notificationParams['correctormailcontent'];
			...
			{			
				...
				$delivery_array["correctormailsubject"]=$this->delivery_creation->prod_step3['correctormailsubject'];
				$delivery_array["correctormailcontent"]=$this->delivery_creation->prod_step3['correctormailcontent'];
			...
			if($this->delivery_creation->prod_step1['theme'])
			{
				...
				$delivery_array["ebooker_cat_id"] = $this->delivery_creation->prod_step1['category'];
			...
			if($this->delivery_creation->prod_step1['total_article']>0 && $delivery_identifier)
			{
				...
				for($i=1;$i<=$total_article;$i++)
				{
					...
					if($this->delivery_creation->prod_step1['theme'])
					{
					$article_array["ebooker_cat_id"] = $this->delivery_creation->prod_step1['category'];
                ...
                {                    
					...
					{					
						...
						if($this->delivery_creation->prod_step1['correction']=='external')
						{
							...
							$parameters=array();
			                $parameters['editobject']=$this->delivery_creation->prod_step3['correctormailsubject'];
			                $parameters['editmessage']=$this->delivery_creation->prod_step3['correctormailcontent'];
	...
	{		
		...
		$aowithlink='<a href="http://ep-test.edit-place.com/contrib/aosearch">'.stripslashes($this->delivery_creation->prod_step1['title']).'</a>';
		$sub=$email[0]['Object'];
