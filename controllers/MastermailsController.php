...
class MastermailsController extends Ep_Controller_Action
{
	public function init()
	{
        ...
        $this->attachment_path="/home/sites/site5/web/FO/attachments/";
        $this->sid = session_id();        
...
//if($_REQUEST['debug']){echo '<pre>';    print_r($reply_messages); exit;}
	...
	public function sentMailsAction()
	{
		...
		if($this->user_type=='superadmin')
		{
			...
			$sent_messages= $ticket_obj->getMasterInboxFOUsersFiltered($sentbox_parmas,0, $sentbox_parmas['ep_user_id']);
		...
		//echo $this->user_type."--". $this->user_id;exit;
    ...
    public function getContribSelectAction()
    {
        ...
        $corrector_counts = $contrib_obj->getContribsCount($params,1);
        //
        $client_info = $contrib_obj->getContribsList($params);
        $client_list = array();//echo '<pre>';print_r($client_info);exit;
        ...
        exit($options.'</select> <span><b>'.count($client_list).'</b></span><span id="langspinner" style="display:none;"><img src="/BO/theme/gebo/img/loading.gif"></span>#'.
					...
					if($ticket_details[0]['recipient_id']!=$ep_user_id)
					{
						...
						$object="Nouvel email en service client";						   
							  ...
							  $text_mail="<p>Cher chef de projet,</p>
										<p><b>$admin_user_name</b> vous a assign&eacute; un message car il a consid&eacute;r&eacute; que vous &eacute;tiez plus en mesure d'y r&eacute;pondre. <a href=\"https://admin-ep-test.edit-place.com/mastermails/inbox-ep?submenuId=ML6-SL2\">Cliquez-ci</a> pour le consulter. </p>
