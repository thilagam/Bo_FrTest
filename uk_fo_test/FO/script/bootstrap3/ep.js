...
$('input#optionsRadios2').on('ifUnchecked', function(event){
	  	$("#option1fields").show();
		$("#option2fields").hide();
});

$('input#dontknowcheck').on('ifChecked', function(event){
	   $("[id^=articlenum_]" ).each(function(z) {
			var idtype=this.id.split("_");
			$("#"+this.id).attr('disabled', true);	  
			$("#frequency_"+idtype[1]).attr('disabled', true);	  
		});
});
$('input#dontknowcheck').on('ifUnchecked', function(event){
	  	 $("[id^=articlenum_]" ).each(function(z) {
			var idtype=this.id.split("_");
			$("#"+this.id).removeAttr('disabled');	
			$("#frequency_"+idtype[1]).removeAttr('disabled');		
		});
});
