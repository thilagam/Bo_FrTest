...
function PhpUploader_GetSystemTempFolder()
{
	...
	$str = '/home/sites/site6/users/tmp/';
...
class PhpUploader
{
	...
	function GetString()
	{
		...
		//if($this->LoadJS)$code.="<script type='text/javascript' src='$resourcehandler?type=script'></script>";
		if ($this->LoadJS)
			$code.="<script type='text/javascript' src='/dapplication/script/uploader.js'></script>";
		
		$code.="<img AdvancedOptionNoSilverlight='1' id='" . $this->Name . "_Loader' UniqueID='" . $this->Name . "' ";
	...
	function InternalGetTempDirectory()
	{
		...
		$dir = '/home/sites/site6/users/tmp/uploadertemp';
		if($dir==null)
	...
	function _SaveSecuritySetting($name)
	{
		$_SESSION["Uploader_val_" . $name]=$this->$name;
		$_SESSION["Uploader_set_" . $name]="1";
	}
	function _LaveSecuritySetting($name)
	{
		if(@$_SESSION["Uploader_set_" . $name])
		{
			$this->$name=$_SESSION["Uploader_val_" . $name];
		}
	}
	...
	function _HandleXhttpUpload()
	{
		...
		if($ps==0)
		{
			...
			$dir = $this->InternalGetTempDirectory();
			$filepath="$dir/uploading." . $this->_fileguid . "." . PhpUploader_GetBaseName($fn) . ".resx";
...
class PhpUploadFile
{
	...
	public $FileSize;
	public $FilePath;
	function MoveTo($newpath,$newfilename)
	{
		if(is_dir($newpath))
